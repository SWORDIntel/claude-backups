...
detect_installation_mode() {
    ...
    if [ -f "$SCRIPT_DIR/statusline.lua" ]; then
...
create_wrapper() {
    ...
    if [ "$INSTALL_ORCHESTRATION" = true ] && [ -f "$SCRIPT_DIR/claude-unified" ]; then
...
show_status() {
    ...
    if [ -f "$SCRIPT_DIR/claude-orchestration-bridge.py" ] || [ -f "$USER_BIN_DIR/claude-orchestration-bridge.py" ]; then
