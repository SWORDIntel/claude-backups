...
try:
    ...
    from permission_fallback_system import (
        PermissionFallbackSystem,
        handle_restricted_request,
    )
...
class ClaudeUniversalOptimizer:
    ...
    def _initialize_optimizations(self):
        ...
        try:
            ...
            self.token_optimizer = TokenOptimizer(
                cache_size=1000, ttl_seconds=self.config["cache_ttl"]
            )
            ...
            logger.info(
                f"✓ Permission system initialized ({self.permission_system.capabilities.permission_level.value})"
            )
    ...
    def optimize_request(self, args: List[str]) -> Tuple[List[str], Dict[str, Any]]:
        ...
        try:
            ...
            if self.token_optimizer and operation.get("content"):
                ...
                optimized_content = self.token_optimizer.compress_response(
                    original_content
                )

                if len(optimized_content) < len(original_content):
                    metadata["optimizations_applied"].append("token_optimization")
                    metadata["tokens_saved"] += len(original_content) - len(
                        optimized_content
                    )
    ...
    def _detect_operation(self, args: List[str]) -> Dict[str, Any]:
        ...
        # Detect query operations
            ...
            keyword in arg_string.lower()
            for keyword in ["what", "how", "why", "when", "where"]
        ):
    ...
    def _update_args_content(
        self, args: List[str], optimized_content: str
    ) -> List[str]:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "average_tokens_saved": self.stats["tokens_saved"]
            / max(1, self.stats["requests_processed"]),
        }
...
def main():
    ...
    if metadata.get("optimizations_applied"):
        logger.info(
            f"Applied optimizations: {', '.join(metadata['optimizations_applied'])}"
        )
