...
class SystemCapability:
    ...
    def _check_c_layer(self) -> Dict[str, Any]:
        ...
        try:
            ...
            c_processes = [
                line
                for line in lines
                if ("agent_bridge" in line or "ultra_hybrid" in line)
            ...
            binary_path = (
                agents_root / "binary-communications-system" / "ultra_hybrid_enhanced"
            )
            binary_exists = (
                binary_path.exists()
                and binary_path.is_file()
                and not str(binary_path).endswith(".c")
            )
    ...
    def _check_tandem(self) -> Dict[str, Any]:
        ...
        orchestrator_path = (
            agents_root / "src" / "python" / "production_orchestrator.py"
        )
        ...
        return {
            ...
            "orchestrator_path": (
                str(orchestrator_path) if orchestrator_path.exists() else None
            ),
        }
    ...
    def _check_thermal(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if temps:
                max_temp = max(
                    sensor.current for sensors in temps.values() for sensor in sensors
                )
                state = (
                    "CRITICAL"
                    if max_temp > 95
                    else "HIGH" if max_temp > 85 else "NORMAL"
                )
...
class GlobalAgentCoordinator:
    ...
    def __init__(self):
        ...
        self.cache_file = (
            Path.home() / ".cache" / "claude-agents" / "unified_discovery.json"
        )
        self.config_file = (
            Path.home() / ".cache" / "claude-agents" / "coordination_config.json"
        )

        # Component paths (relative to agents_dir)
        self.orchestrator_path = (
            self.agents_dir / "src" / "python" / "production_orchestrator.py"
        )
        self.tandem_path = (
            self.agents_dir / "src" / "python" / "production_orchestrator.py"
        )
        self.c_bridge_path = (
            self.agents_dir / "binary-communications-system" / "ultra_hybrid_enhanced"
        )
    ...
    def _display_capabilities(self, caps: Dict[str, Any]):
        ...
        print(
            f"  • Tandem Orchestrator: {'✓' if caps['tandem']['available'] else '✗'} "
            f"{'Available' if caps['tandem']['available'] else 'Not found'}"
        )
        print(
            f"  • C Binary Layer: {'✓' if caps['c_layer']['available'] else '✗'} "
            f"{'Running' if caps['c_layer']['available'] else 'Offline (Python fallback)'}"
        )
        print(
            f"  • AVX-512: {'✓' if caps['avx512'] else '✗'} "
        ...
        for agent_file in agent_files:
            ...
            if agent_data:
                discovered[agent_name.lower()] = {
                    "name": agent_name,
                    "path": str(
                        agent_file.relative_to(self.agents_dir)
                        if self.agents_dir in agent_file.parents
                        else agent_file
                    ),
                    "metadata": agent_data.get("metadata", {}),
                    "capabilities": self._extract_capabilities(agent_data),
                    "description": self._extract_description(agent_data),
                    "category": agent_data.get("metadata", {}).get(
                        "category", "GENERAL"
                    ),
                    "status": agent_data.get("metadata", {}).get(
                        "status", "PRODUCTION"
                    ),
    ...
    def _extract_capabilities(self, agent_data: Dict) -> List[str]:
        ...
        if "SECURITY" in category:
            capabilities.extend(
                ["security_analysis", "vulnerability_assessment", "compliance_check"]
            )
    ...
    def _determine_execution_path(self, agent_info: Dict, mode: ExecutionMode) -> str:
        ...
        if mode == ExecutionMode.INTELLIGENT:
            if self.capabilities.c_layer_available and agent_info["category"] in [
                "CRITICAL",
                "PERFORMANCE",
            ]:
                return "C_BRIDGE"
    ...
    def _invoke_via_task_tool(self, agent_name: str, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            task_request = {
                ...
                "prompt": prompt,
                "priority": "NORMAL",
                "context": {"invoked_via": "Global Agent Bridge", "mode": "TASK_TOOL"},
            }
    ...
    def _invoke_via_tandem(
        self, agent_name: str, prompt: str, mode: ExecutionMode
    ) -> Dict[str, Any]:
    ...
    def _invoke_via_c_bridge(self, agent_name: str, prompt: str) -> Dict[str, Any]:
        ...
        try:
            ...
            request_data = json.dumps(
                {"agent": agent_name, "prompt": prompt, "timestamp": time.time()}
            ).encode()
    ...
    def _setup_task_tool_integration(self):
        ...
        wrapper_file = (
            Path.home() / ".local" / "share" / "claude-agents" / "task_tool_bridge.py"
        )
...
'''
    ...
    def _initialize_tandem(self):
        ...
        tandem_config = {
            ...
            "c_bridge_path": (
                str(self.c_bridge_path) if self.c_bridge_path.exists() else None
            ),
            "performance_targets": {
    ...
    def _save_configuration(self):
        ...
        config = {
            ...
            "metrics": {
                "avg_response_time": (
                    sum(self.metrics["avg_response_time"])
...
def main():
    ...
    parser.add_argument(
        "--install", action="store_true", help="Install global integration"
    )
