...
class HookPerformanceMonitor:
    ...
    def record_execution(self, execution_time_ms: float, success: bool = True, 
                        cache_hit: bool = False, agent_count: int = 1):
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        if not self.execution_times:
            return {'status': 'no_data', 'uptime_seconds': time.time() - self.start_time}
        ...
        return {
            ...
            'success_rate': (successful_execs / total_execs * 100) if total_execs > 0 else 0,
            'error_count': self.error_counts,
            'performance': {
                'avg_execution_time_ms': sum(recent_times) / len(recent_times) if recent_times else 0,
                ...
                'p95_execution_time_ms': sorted(recent_times)[int(len(recent_times) * 0.95)] if len(recent_times) > 5 else 0
            },
            'cache_performance': {
