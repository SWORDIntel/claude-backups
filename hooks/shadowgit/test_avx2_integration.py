...
def test_avx2_diff_engine():
    ...
    lib.shadowgit_avx2_diff.argtypes = [
        ctypes.c_char_p,
        ctypes.c_char_p,
        ctypes.c_size_t,
    ]
    ...
    lib.shadowgit_avx2_diff_buffers.argtypes = [
        ctypes.c_char_p,
        ctypes.c_char_p,
        ctypes.c_size_t,
    ]
    ...
    start = time.time()
    diff_count = lib.shadowgit_avx2_diff(
        content1, content2, min(len(content1), len(content2))
    )
    ...
    start = time.time()
    diff_count2 = lib.shadowgit_avx2_diff_buffers(
        content1, content2, min(len(content1), len(content2))
    )
...
"""
    ...
    os.environ["SHADOWGIT_AVX2_PATH"] = str(
        Path.home() / "shadowgit" / "c_src_avx2" / "bin" / "libshadowgit_avx2.so"
    )
    ...
    try:
        ...
        from shadowgit_unified_system import UnifiedConfig, UnifiedSystem
