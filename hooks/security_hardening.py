...
class SecurityValidator:
    ...
    def validate_input(cls, input_text: str) -> tuple[bool, str, Optional[str]]:
        ...
        if len(input_text) > cls.MAX_INPUT_SIZE:
            return False, "", f"Input exceeds maximum size of {cls.MAX_INPUT_SIZE} characters"
        ...
        if isinstance(result, dict):
            for key, value in result.items():
                if isinstance(value, str):
                    ...
                    # Remove stack traces
                    value = re.sub(r'File ".*?", line \d+', 'File "<hidden>", line ***', value)
...
async def test_security():
    ...
    for input_text, should_pass, description in test_cases:
        result = await hooks.process(input_text[:50] if len(input_text) > 50 else input_text)
    ...
    blocked = [r for r in results if isinstance(r, dict) and r.get('status') == 'rejected']
