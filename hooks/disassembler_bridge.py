...
class IntegrationTester:
    ...
    def test_framework_detection(self):
        ...
        if ghidra_script.exists():
            self.test_results.append(("Framework Detection", "PASS", "ghidra-integration.sh found"))
            
            # Check if executable
            if os.access(str(ghidra_script), os.X_OK):
                self.test_results.append(("Script Executable", "PASS", "Script has execute permission"))
            else:
                self.test_results.append(("Script Executable", "FAIL", "Script not executable"))
        else:
            self.test_results.append(("Framework Detection", "FAIL", f"ghidra-integration.sh not found at {ghidra_script}"))
        
        # Check Ghidra availability
        status = self.hook.get_status()
        if status['ghidra_available']:
            self.test_results.append(("Ghidra Detection", "PASS", f"Ghidra found: {status.get('ghidra_type', 'unknown')}"))
        else:
            self.test_results.append(("Ghidra Detection", "WARN", "Ghidra not detected - analysis limited"))
    ...
    def test_binary_detection(self):
        ...
        if detected > 0:
            self.test_results.append(("Binary Detection", "PASS", f"Detected {detected}/{len(test_paths)} system binaries"))
        else:
            self.test_results.append(("Binary Detection", "FAIL", "Could not detect system binaries"))
    ...
    def create_test_binary(self) -> Path:
        ...
        try:
            ...
            if result.returncode == 0:
                self.test_results.append(("Test Binary Creation", "PASS", "Compiled test binary"))
                return binary_file
            else:
                self.test_results.append(("Test Binary Creation", "FAIL", f"Compilation failed: {result.stderr.decode()}"))
                return None
                
        except Exception as e:
            self.test_results.append(("Test Binary Creation", "SKIP", f"GCC not available: {e}"))
    ...
    def test_hook_analysis(self, binary_path: Path):
        ...
        try:
            ...
            if not self.hook.is_binary(str(binary_path)):
                self.test_results.append(("Hook Binary Detection", "FAIL", f"Failed to detect {binary_path.name} as binary"))
                return
            
            self.test_results.append(("Hook Binary Detection", "PASS", f"Correctly identified {binary_path.name} as binary"))
            
            # Check if should analyze
            if self.hook.should_analyze(str(binary_path)):
                self.test_results.append(("Hook Analysis Check", "PASS", "File marked for analysis"))
            else:
                self.test_results.append(("Hook Analysis Check", "INFO", "File already cached or not eligible"))
            
            # Try analysis (may fail if ghidra not installed)
            result = self.hook.analyze_with_ultrathink(str(binary_path), mode='static')
            
            if result['status'] == 'completed':
                self.test_results.append(("ULTRATHINK Analysis", "PASS", f"Analysis completed successfully"))
                
                # Check for results
                if result.get('phases_completed'):
                    self.test_results.append(("Phase Completion", "PASS", f"Completed {len(result['phases_completed'])} phases"))
                if result.get('threat_score') is not None:
                    self.test_results.append(("Threat Scoring", "PASS", f"Threat score: {result['threat_score']}"))
                if result.get('meme_score') is not None:
                    self.test_results.append(("Meme Scoring", "PASS", f"Meme score: {result['meme_score']}"))
                    
            elif result['status'] == 'error' and 'ghidra-integration.sh not found' in result.get('error', ''):
                self.test_results.append(("ULTRATHINK Analysis", "SKIP", "ghidra-integration.sh not installed"))
            else:
                self.test_results.append(("ULTRATHINK Analysis", "FAIL", f"Analysis failed: {result.get('error', 'Unknown error')}"))
    ...
    def test_bridge_processing(self, binary_path: Path):
        ...
        try:
            ...
            if result['status'] == 'completed':
                self.test_results.append(("Bridge Processing", "PASS", "Binary processed successfully"))
                ...
                self.test_results.append(("Bridge Processing", "SKIP", result.get('reason', 'Unknown reason')))
            else:
                self.test_results.append(("Bridge Processing", "FAIL", result.get('error', 'Unknown error')))
    ...
    def test_cache_functionality(self):
        ...
        try:
            ...
            if self.hook.cache_file.exists():
                ...
                self.test_results.append(("Cache System", "PASS", f"Cache operational with {cache_size} entries"))
            else:
                self.test_results.append(("Cache System", "INFO", "Cache file not yet created"))
    ...
    def print_results(self):
        ...
        print("\n" + "-" * 60)
        print(f"PASSED: {pass_count} | FAILED: {fail_count} | SKIPPED: {skip_count} | WARNINGS: {warn_count}")
        print("-" * 60)
