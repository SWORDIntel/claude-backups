...
class EnhancedNaturalInvocationHook:
    ...
    def _initialize_pattern_triggers(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            # ORCHESTRATORS
            'director': {
                'patterns': ['coordinate', 'manage', 'orchestrate', 'plan', 'strategic', 'roadmap'],
                'regex': [r'high[\s-]?level', r'big[\s-]?picture', r'overall\s+strategy'],
                'priority': 5
            },
            'projectorchestrator': {
                'patterns': ['project', 'workflow', 'pipeline', 'process', 'tasks', 'coordinate'],
                'regex': [r'multi[\s-]?agent', r'task\s+coordination'],
                'priority': 4
            },
            'redteamorchestrator': {
                'patterns': ['red team', 'penetration', 'attack simulation', 'adversarial'],
            ...
            # SECURITY
            'cso': {
                'patterns': ['security', 'audit', 'vulnerability', 'compliance', 'risk'],
            ...
            'securityauditor': {
                'patterns': ['security audit', 'code review', 'compliance', 'verification'],
            ...
            'cryptoexpert': {
                'patterns': ['crypto', 'encryption', 'decrypt', 'cipher', 'certificate', 'ssl'],
            ...
            'bastion': {
                'patterns': ['bastion', 'jumphost', 'ssh', 'access control', 'perimeter'],
            ...
            # INTELLIGENCE
            'nsa': {
                'patterns': ['sigint', 'signals', 'intelligence', 'intercept', 'surveillance'],
            ...
            # DEVELOPMENT
            'leadengineer': {
                'patterns': ['lead', 'technical lead', 'architecture', 'code review', 'mentor'],
                'regex': [r'lead\s+engineer', r'technical\s+lead'],
                'priority': 4
            },
            'constructor': {
                'patterns': ['build', 'create', 'scaffold', 'generate', 'setup', 'construct'],
                'regex': [r'create\s+(?:new\s+)?(?:project|app)', r'scaffold(?:ing)?'],
                'priority': 3
            },
            'debugger': {
                'patterns': ['debug', 'trace', 'investigate', 'troubleshoot', 'diagnose', 'error'],
            ...
            'linter': {
                'patterns': ['lint', 'format', 'style', 'clean', 'beautify', 'standards'],
            ...
            # UI/FRONTEND
            'web': {
                'patterns': ['web', 'frontend', 'react', 'vue', 'angular', 'html', 'css'],
            ...
            'tui': {
                'patterns': ['tui', 'terminal ui', 'console', 'ncurses', 'cli interface'],
            ...
            # DATA & ML
            'datascience': {
                'patterns': ['data science', 'analysis', 'statistics', 'visualization', 'pandas'],
            ...
            'researcher': {
                'patterns': ['research', 'investigate', 'explore', 'study', 'literature'],
            ...
            # INFRASTRUCTURE
            'infrastructure': {
                'patterns': ['infrastructure', 'terraform', 'kubernetes', 'k8s', 'cloud'],
            ...
            'optimizer': {
                'patterns': ['optimize', 'performance', 'speed', 'efficiency', 'benchmark'],
                'regex': [r'optimiz(?:e|ation)', r'performance\s+tuning'],
                'priority': 4
            },
            'oversight': {
                'patterns': ['oversight', 'audit', 'compliance', 'governance', 'policy'],
            ...
            'integration': {
                'patterns': ['integration', 'integrate', 'connect', 'bridge', 'interface'],
            ...
            # HARDWARE
            'npu': {
                'patterns': ['npu', 'neural processing', 'ai acceleration', 'inference'],
    ...
    def _initialize_workflow_patterns(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            'security_assessment': {
                'triggers': ['security audit', 'vulnerability assessment', 'compliance check'],
                'agents': ['cso', 'securityauditor', 'security', 'cryptoexpert'],
                'coordinator': 'cso',
                'confidence_boost': 0.2
            },
            'incident_response': {
                'triggers': ['production down', 'emergency', 'critical issue', 'outage'],
            ...
            'full_stack_development': {
                'triggers': ['full stack', 'end to end application', 'frontend and backend'],
                'agents': ['web', 'apidesigner', 'database', 'infrastructure'],
                'coordinator': 'leadengineer',
                'confidence_boost': 0.15
            },
            'ml_pipeline': {
                'triggers': ['ml pipeline', 'machine learning deployment', 'model training'],
    ...
    def analyze_input(self, user_input: str) -> InvocationContext:
        ...
        if self.fuzzy_matcher and ENHANCED_MATCHING:
            try:
                ...
                context.execution_mode = results.get('execution_mode', ExecutionMode.INTELLIGENT)
        ...
        for agent_name, config in self.pattern_triggers.items():
            ...
            for pattern in config.get('patterns', []):
                if pattern in input_lower:
                    ...
                    pattern_score = (1 - position / len(input_lower)) * (len(pattern) / 20)
    ...
    def _find_explicit_mentions(self, user_input: str) -> List[str]:
        ...
        for agent_name in self.pattern_triggers.keys():
            ...
            elif re.search(rf'\b(?:use|invoke|call|activate)\s+(?:the\s+)?{re.escape(agent_name)}\b', input_lower):
                mentioned.append(agent_name)
    ...
    mode="{context.execution_mode.value}"
        ...
        if context.workflow:
            coordinator = self.workflow_patterns.get(context.workflow, {}).get('coordinator', 'projectorchestrator')
    ...
    def suggest_invocation(self, user_input: str, threshold: float = 0.6) -> Optional[str]:
        """
    ...
    def get_agent_info(self, agent_name: str) -> Dict[str, Any]:
        ...
        if not agent_file.exists():
            ...
            for file in self.agents_root.glob("*.md"):
                if file.stem.lower() == agent_name.lower().replace('-', '').replace('_', ''):
                    agent_file = file
                    break
        ...
        if agent_file.exists():
            ...
            try:
                ...
                if 'capabilities:' in content:
                    ...
                    if caps_match:
                        caps = [c.strip().strip('"\'') for c in caps_match.group(1).split(',')]
...
def hook_post_edit(context: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if file_path.endswith('.py'):
        suggestions.append("Consider 'python-internal' agent for Python-specific optimization")
    elif file_path.endswith('.rs'):
        suggestions.append("Consider 'rust-internal' agent for Rust memory safety checks")
    ...
    # General suggestions
    if any(file_path.endswith(ext) for ext in ['.py', '.js', '.ts', '.go', '.rs', '.java']):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Enhanced Natural Agent Invocation Hook")
