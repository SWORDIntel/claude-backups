...
PATTERNS_FILE = os.environ.get(
    "CLAUDE_PATTERNS_FILE", BASE_PATH / "agent-invocation-patterns.yaml"
)
...
class EnhancedSemanticMatcher:
    ...
    def __init__(self, patterns_file: Optional[str] = None):
        ...
        if self.patterns_file.exists():
            ...
            logger.info(
                f"Patterns file not found at {self.patterns_file}, using defaults"
            )
        ...
        return {
            ...
            "compound_patterns": {
                ...
                "performance_optimization": {
                    "required": [
                        "performance OR speed OR optimize",
                        "slow OR fast OR latency",
                    ],
    ...
    def _build_concept_graph(self) -> Dict[str, Set[str]]:
        ...
        for cluster_name, cluster in self.semantic_clusters.items():
            all_words = (
                cluster.core + cluster.related + cluster.action + cluster.context
            )
    ...
    def calculate_semantic_similarity(
        self, text: str, target: str, use_graph: bool = True
    ) -> float:
        """
        ...
        for pattern_name, pattern_config in workflow_patterns.items():
            ...
            for trigger in pattern_config["triggers"]:
                similarity = self.calculate_semantic_similarity(
                    input_lower, trigger, use_graph=True
                )
            ...
            if max_similarity > 0.6:
                ...
                detected_patterns.append(
                    (pattern_name, pattern_config["agents"], confidence)
                )
            ...
            for pattern_name, pattern_config in self.patterns[
                ...
                for req_term in required:
                    if " OR " in req_term:
                        ...
                        if (
                            self.calculate_semantic_similarity(input_lower, req_term)
                            > 0.5
                        ):
                            required_matches += 1
                ...
                if required_matches > 0 and required:
                    ...
                    for opt_term in optional:
                        if (
                            self.calculate_semantic_similarity(input_lower, opt_term)
                            > 0.5
                        ):
    ...
    def extract_explicit_invocations(
        self, user_input: str
        ...
        for pattern, confidence in invocation_patterns:
            ...
            for match in matches:
                ...
                for agent_name in self.agent_expertise.keys():
                    similarity = self.calculate_semantic_similarity(
                        agent_name.lower(), target
                    )
                    if similarity > 0.7:
                        invocations.append(
                            (agent_name, target, confidence * similarity)
                        )
    ...
    def get_contextual_recommendations(
        ...
        for agent, score in sorted_agents:
            ...
            primary_reason = (
                agent_reasons[0] if agent_reasons else "Contextual relevance"
            )
...
class EnhancedAgentMatcher:
    ...
    def match(self, user_input: str) -> Dict[str, Any]:
        ...
        # Get recommendations
        recommendations = self.semantic_matcher.get_contextual_recommendations(
            user_input
        )
...
if __name__ == "__main__":
    ...
    if args.agents:
        ...
        for agent, score in sorted(
            results["semantic_agents"].items(), key=lambda x: x[1], reverse=True
        )[:5]:
