...
class ClaudeCodeHookAdapter:
    ...
    def _handle_pre_task(self) -> List[Dict[str, Any]]:
        ...
        actions.append(
            {"type": "environment_setup", "status": "success", "details": env_setup}
        )
        ...
        actions.append(
            {"type": "agent_selection", "status": "success", "agents": selected_agents}
        )
    ...
    def _handle_post_edit(self) -> List[Dict[str, Any]]:
        ...
        if "edited_files" in self.context:
            for file in self.context["edited_files"]:
                actions.append(
                    {"type": "file_processed", "file": file, "status": "success"}
                )
        ...
        actions.append(
            {"type": "cache_update", "status": "success", "details": cache_result}
        )
    ...
    def _handle_post_task(self) -> List[Dict[str, Any]]:
        ...
        actions.append(
            {"type": "cleanup", "status": "success", "details": cleanup_result}
        )

        # 2. Generate report
        report = self._generate_report()
        actions.append(
            {"type": "report_generation", "status": "success", "report": report}
        )
        ...
        if self.context.get("claude_task_id"):
            ...
            actions.append(
                {"type": "archive", "status": "success", "details": archive_result}
            )
    ...
    def _setup_environment(self) -> Dict[str, Any]:
        ...
        setup = {"directories_created": [], "paths_added": [], "environment_vars": {}}
    ...
    def _cache_results(self) -> Dict[str, Any]:
        ...
        cache_file = (
            cache_dir / f"task_{self.context.get('claude_task_id', 'unknown')}.json"
        )
    ...
    def _cleanup_environment(self) -> Dict[str, Any]:
        ...
        cleanup = {"temp_files_removed": 0, "cache_cleared": False}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--register", action="store_true", help="Register with Claude Code"
    )
