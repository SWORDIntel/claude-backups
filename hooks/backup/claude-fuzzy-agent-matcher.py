    ...
    def _initialize_agents(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "projectorchestrator": {
                ...
                "patterns": [
                    r"coordinate\s+(?:agents|tasks)",
                    r"manage\s+dependencies",
                ],
            },
            "redteamorchestrator": {
            ...
            "cso": {
                ...
                "patterns": [
                    r"security\s+(?:audit|review|assessment)",
            ...
            "quantumguard": {
                ...
                "patterns": [
                    r"quantum\s+(?:resistant|safe|security)",
                    r"post.?quantum",
                ],
            },
            "bastion": {
            ...
            "nsa": {
                ...
                "patterns": [
                    r"signal\s+intelligence",
                    r"network\s+analysis",
                    r"traffic\s+analysis",
                ],
            },
            "apt41-defense-agent": {
                "type": "THREAT_DEFENSE",
            ...
            "bgp-purple-team-agent": {
                ...
                "patterns": [
                    r"bgp\s+(?:security|hijack)",
                    r"purple\s+team",
                    r"routing\s+security",
                ],
            },
            "psyops_agent": {
                ...
                "patterns": [
                    r"psy(?:chological)?\s+ops",
                    r"social\s+engineering",
                    r"influence\s+campaign",
                ],
            },
            # DEVELOPMENT AGENTS
            "leadengineer": {
            ...
            "constructor": {
                ...
                "patterns": [
                    r"generate\s+(?:code|project)",
                    r"scaffold(?:ing)?",
                    r"create\s+(?:project|app)",
                ],
            },
            "debugger": {
                "type": "DEVELOPMENT",
            ...
            "web": {
                ...
                "patterns": [
                    r"(?:web|frontend)\s+(?:app|development)",
                    r"react|vue|angular",
                ],
            },
            "androidmobile": {
            ...
            "iot-access-control-agent": {
                ...
                "patterns": [
                    r"iot|internet\s+of\s+things",
                    r"device\s+management",
                    r"mqtt",
                ],
            },
        }
...
class ClaudeFuzzyMatcher:
    ...
    def _initialize_workflow_patterns(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            ...
            "deployment_pipeline": {
                "patterns": [
                    r"deploy(?:ment)?\s+to\s+production",
                    r"release\s+pipeline",
                ],
    ...
    def match(self, user_input: str) -> Dict[str, Any]:
        ...
        if explicit_agents:
            ...
            results["reasoning"].append(
                f"Explicit agent mention: {', '.join(explicit_agents)}"
            )
            return results
        ...
        if self.semantic_matcher:
            try:
                ...
                semantic_matches = list(
                    semantic_results.get("semantic_agents", {}).keys()
                )[:3]
            except:
                pass
        ...
        all_matches = self._combine_matches(
            pattern_matches, keyword_matches, semantic_matches
        )
        ...
        results["execution_mode"] = self._determine_execution_mode(
            user_input, results["matched_agents"]
        )
    ...
    def _check_explicit_mentions(self, input_lower: str) -> List[str]:
        ...
        for agent_name in self.registry.agents.keys():
            if (
                agent_name.lower() in input_lower
                or agent_name.replace("-", "").lower() in input_lower
            ):
    ...
    def _generate_reasoning(
        self, matches: Dict[str, float], user_input: str
    ) -> List[str]:
    ...
    def _determine_execution_mode(
        ...
        if any(
            word in input_lower
            for word in ["urgent", "critical", "emergency", "asap", "performance"]
        ):
        ...
        if any(
            agent in agents
            for agent in ["cso", "securityauditor", "cryptoexpert", "nsa"]
        ):
    ...
    if args.list:
        ...
        for agent_type, agents in registry.category_map.items():
            ...
            for agent in sorted(agents):
                ...
                print(
                    f"  • {agent:25} Priority: {agent_data['priority'].name:8} Capabilities: {len(agent_data['capabilities'])}"
                )
