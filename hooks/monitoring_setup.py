...
class HealthChecker:
    ...
    async def check_performance(self) -> Dict[str, Any]:
        ...
        tasks = [self.hooks.process(f"Test input {i}") for i in range(test_count)]
    ...
    async def check_error_rate(self) -> Dict[str, Any]:
        ...
        return {"healthy": True, "error_rate": 0.0, "errors_last_5min": 0}
...
class AlertManager:
    ...
    def check_thresholds(self, metrics: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if (
            ...
            and metrics["cpu_percent"] > self.alert_thresholds["cpu_percent"]
        ):
            alerts.append(
                {
                    "severity": "warning",
        ...
        # Check Memory
        if (
            "memory_percent" in metrics
            and metrics["memory_percent"] > self.alert_thresholds["memory_percent"]
        ):
            alerts.append(
                {
                    "severity": "critical",
        ...
        if (
            ...
            and metrics["throughput"] < self.alert_thresholds["throughput_min"]
        ):
            alerts.append(
                {
                    "severity": "warning",
...
class MonitoringDashboard:
    ...
    def __init__(
        self, metrics: MetricsCollector, health: HealthChecker, alerts: AlertManager
    ):
    ...
    def generate_report(self) -> str:
        ...
        if self.alerts.alert_history:
            for alert in list(self.alerts.alert_history)[-5:]:  # Last 5 alerts
                report.append(
                    f"  [{alert.get('severity', 'info').upper()}] {alert.get('message', 'Unknown alert')}"
                )
        else:
...
async def setup_monitoring(hooks_system):
    ...
    logger.info(
        f"Initial health check: {'PASSED' if health_results['overall_healthy'] else 'FAILED'}"
    )
