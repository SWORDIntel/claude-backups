/*
 * MLOPS AGENT - Communication System Integration
...
// Initialize agent
int mlops_init(mlops_agent_t* agent) {
    // Initialize communication context
    agent->comm_context = ufp_create_context("mlops");
    if (!agent->comm_context) {
        return -1;
    }
    
    strcpy(agent->name, "mlops");
    agent->state = AGENT_STATE_ACTIVE;
    
    // Register with discovery service
    agent_register("mlops", AGENT_TYPE_MLOPS, NULL, 0);
    
    return 0;
}

// Process incoming message
int mlops_process_message(mlops_agent_t* agent, ufp_message_t* msg) {
    // TODO: Implement agent-specific logic
    printf("MLOps received message from %s\n", msg->source);
