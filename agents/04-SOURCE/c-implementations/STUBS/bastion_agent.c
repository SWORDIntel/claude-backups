/*
 * BASTION AGENT - Communication System Integration
...
// Initialize agent
int bastion_init(bastion_agent_t* agent) {
    // Initialize communication context
    agent->comm_context = ufp_create_context("bastion");
    if (!agent->comm_context) {
        return -1;
    }
    
    strcpy(agent->name, "bastion");
    agent->state = AGENT_STATE_ACTIVE;
    
    // Register with discovery service
    agent_register("bastion", AGENT_TYPE_BASTION, NULL, 0);
