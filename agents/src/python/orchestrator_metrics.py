...
class SystemMetricsCollector(MetricsCollector):
    ...
    async def collect(self) -> SystemMetrics:
        ...
        network_io = {
            ...
            'packets_sent': network_stats.packets_sent - self.network_stats_prev.packets_sent,
            'packets_recv': network_stats.packets_recv - self.network_stats_prev.packets_recv,
        }
...
class AgentMetricsCollector(MetricsCollector):
    ...
    def update_agent_resources(self, agent_name: str, cpu_usage: float, memory_usage: float):
...
class PrometheusExporter:
    ...
    def update_agent_metrics(self, agent_metrics: Dict[str, AgentMetrics]):
        ...
        for agent_name, metrics in agent_metrics.items():
            ...
            # Update gauges
            self.agent_health_score.labels(agent_name=agent_name).set(metrics.health_score)
            self.agent_cpu_usage.labels(agent_name=agent_name).set(metrics.cpu_usage)
            self.agent_memory_usage.labels(agent_name=agent_name).set(metrics.memory_usage)
    ...
    def update_system_metrics(self, system_metrics: SystemMetrics):
        ...
        for i, period in enumerate(['1m', '5m', '15m']):
            if i < len(system_metrics.load_average):
                self.system_load_average.labels(period=period).set(system_metrics.load_average[i])
    ...
    def update_communication_metrics(self, comm_metrics: CommunicationMetrics):
        ...
        self.communication_messages.labels(status='success')._value._value = comm_metrics.successful_messages
        self.communication_messages.labels(status='failure')._value._value = comm_metrics.failed_messages
...
class MetricsHTTPHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if parsed_path.path == '/metrics':
            ...
            try:
                metrics_data = self.metrics_manager.prometheus_exporter.generate_metrics()
            ...
            try:
                ...
                self.send_header('Content-Type', 'application/json')
                self.end_headers()
                self.wfile.write(json.dumps(status_data, indent=2, default=str).encode('utf-8'))
            except Exception as e:
...
class OrchestratorMetricsManager:
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "system": {
                "cpu_usage": self.system_history[-1].cpu_usage if self.system_history else 0,
                "memory_usage": self.system_history[-1].memory_usage if self.system_history else 0
            }
        }
    ...
    def get_detailed_status(self) -> Dict[str, Any]:
        ...
        return {
            "timestamp": datetime.now().isoformat(),
            "system_metrics": asdict(self.system_history[-1]) if self.system_history else {},
            "agent_metrics": {
                name: asdict(metrics) 
