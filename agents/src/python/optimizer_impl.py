        ...
        if name in self.baselines:
            if value > self.baselines[name] * 1.1:  # 10% regression threshold
                self.regressions.append({
                    ...
                    'current': value,
                    'regression': (value - self.baselines[name]) / self.baselines[name] * 100
                })
...
class OPTIMIZERPythonExecutor:
    ...
    async def profile_code(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            profile_type = payload.get('type', 'comprehensive')  # cpu, memory, line, comprehensive
            ...
            await self._create_enhanced_performance_plan(analysis, target_file, patterns)
            ...
            return {"error": f"Profiling failed: {str(e)}", "traceback": traceback.format_exc()}
            
    async def _profile_function_advanced(self, file_path: str, function_name: str) -> Dict:
        """Advanced function profiling with call graph tracking"""
        try:
            # Import and get the function
            spec = __import__('importlib.util').spec_from_file_location("target_module", file_path)
    ...
    async def _profile_memory(self, file_path: str, function_name: Optional[str]) -> Dict:
    ...
    async def _profile_lines(self, file_path: str, function_name: Optional[str]) -> Dict:
    ...
    def _generate_comprehensive_analysis(self, results: Dict, patterns: List) -> Dict:
        ...
        if 'cpu' in results:
            ...
            self.hot_paths = self._identify_advanced_hot_paths(hot_functions, cpu_stats.get('call_graph', {}))
                    ...
                    if len(parts) >= 6:
                        ...
                        if ':' in file_func and '(' in file_func:
                            ...
                            if ':' in file_part:
                                ...
                                line_num = file_part.split(':')[1] if file_part.split(':')[1].isdigit() else 0
                            else:
                                filename = file_part
                                line_num = 0
                                
                            call_count = int(ncalls.split('/')[0]) if '/' not in ncalls else int(ncalls)
                            
                            result = ProfileResult(
                                function_name=func_part,
                                filename=filename,
                                line_number=int(line_num) if str(line_num).isdigit() else 0,
                                total_time=tottime,
                                cumulative_time=cumtime,
                                call_count=call_count,
                                per_call_time=tottime / max(call_count, 1),
                                percentage=(tottime / max(tottime, 0.001)) * 100 if tottime > 0 else 0,
    ...
    def _identify_advanced_hot_paths(self, functions: List[ProfileResult], call_graph: Dict) -> List[HotPath]:
        ...
        for chain in chains:
            ...
            if chain_functions:
                ...
                hot_path = HotPath(
                    ...
                    percentage=(total_time / max(sum(f.total_time for f in functions), 0.001)) * 100,
                    optimization_potential=self._assess_advanced_potential(chain_functions),
                    priority=self._calculate_advanced_priority(total_time, len(chain_functions), bottleneck),
    ...
    def _determine_bottleneck(self, functions: List[ProfileResult]) -> str:
        ...
        if avg_memory > 100:  # MB
            ...
            return "cpu"
        elif any('io' in f.function_name.lower() or 'read' in f.function_name.lower() 
                or 'write' in f.function_name.lower() for f in functions):
            return "io"
        elif any('lock' in f.function_name.lower() or 'wait' in f.function_name.lower() 
                for f in functions):
            return "lock"
        else:
            return "cpu"
    ...
    def _calculate_advanced_priority(self, total_time: float, num_functions: int, bottleneck: str) -> int:
    ...
    async def _create_enhanced_performance_plan(self, analysis: Dict, target_file: str, patterns: List):
...
"""
    ...
    def _generate_optimization_strategy(self, hot_path: HotPath) -> str:
        ...
        return strategies.get(hot_path.bottleneck_type, "Profile further to identify optimization approach")
    ...
    def _analyze_memory_trend(self, snapshots: List[Dict]) -> str:
        ...
        # Calculate trend
        increases = sum(1 for i in range(1, len(memory_values)) 
    ...
    async def line_profile(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Line-by-line profiling"""
        return await self._profile_lines(
            payload.get('file'),
    ...
    async def profile_async(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "recommendations": ["Consider using asyncio.gather for parallel execution"]
            }
    ...
    def _generate_regression_fixes(self, regressions: List[Dict]) -> List[str]:
        ...
        for reg in regressions:
            if reg['regression'] > 50:
                fixes.append(f"Critical regression in {reg['metric']}: revert recent changes")
            elif reg['regression'] > 20:
                fixes.append(f"Significant regression in {reg['metric']}: profile and optimize")
            else:
    ...
    def _calculate_confidence_interval(self, times: List[float], confidence: float = 0.95) -> Tuple[float, float]:
    ...
    async def handle_error(self, error: Exception, command: Dict[str, Any]) -> Dict[str, Any]:
        ...
        await self._create_optimizer_files(result, context if 'context' in locals() else {})
        return {
            "error": error_msg,
    ...
    def get_recommendations(self) -> List[str]:
        ...
        for key, stats in self.cache_stats.items():
            ...
            if total > 0:
                ...
                if hit_rate < 0.5:
                    recommendations.append(f"Consider removing cache for '{key}' (hit rate: {hit_rate:.1%})")
                elif hit_rate < 0.7:
                    recommendations.append(f"Optimize cache strategy for '{key}' (hit rate: {hit_rate:.1%})")
                    
        return recommendations


    async def _create_optimizer_files(self, result_data: Dict[str, Any], context: Dict[str, Any]):
