    ...
    try:
        ...
        if success:
            ...
            if hasattr(orchestrator, 'agent_registration'):
                ...
                sample_agents = list(orchestrator.agent_registration.registered_agents.keys())[:5]
...
async def test_metrics_collection():
    ...
    try:
        ...
        logger.info(f"   Binary bridge connected: {metrics.get('binary_bridge_connected', False)}")
...
async def run_all_tests():
    ...
    if passed_tests == total_tests:
        logger.info("🎉 ALL TESTS PASSED - Constructor implementation is working correctly!")
        return True
    else:
        logger.warning(f"⚠️ {total_tests - passed_tests} tests failed - implementation needs fixes")
        return False
