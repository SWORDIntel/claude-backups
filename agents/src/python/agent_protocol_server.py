...
class BinaryProtocol:
    ...
    def pack_message(msg_type, payload):
        ...
        header = MAGIC_HEADER + struct.pack('!BBL', PROTOCOL_VERSION, msg_type, len(payload))
        ...
        try:
            while self.running and self.server.running:
                ...
                try:
                    ...
                    while self.buffer:
                        msg_type, payload, self.buffer = BinaryProtocol.unpack_message(self.buffer)
                    ...
                    # Send heartbeat
                    if not self.send_message(MessageType.HEARTBEAT, {'timestamp': time.time()}):
                        break
                except Exception as e:
