...
class HybridBridgeManager:
    ...
    async def initialize(self):
        ...
        logger.info(f"Hybrid Bridge initialized - Primary: {self.primary_system}, Fallback: {self.fallback_system}")
    ...
    async def _configure_systems(self):
        ...
        if Path('/tmp/.s.PGSQL.5432').exists() or Path('/var/run/postgresql/.s.PGSQL.5432').exists():
    ...
    async def _health_check_system(self, system_name: str, config: SystemConfig):
        ...
        try:
            ...
            logger.info(f"✓ {system_name.upper()} system healthy (score: {config.health_score:.1f}, response: {response_time:.3f}s)")
    ...
    def _select_primary_system(self):
        ...
        available_systems = [(name, config) for name, config in self.systems.items() if config.available]
    ...
    async def route_query(self, query_type: str, query_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            result = await self._execute_on_system(self.primary_system, query_type, query_data)
            ...
            if self.fallback_system:
                try:
                    result = await self._execute_on_system(self.fallback_system, query_type, query_data)
                    result['system'] = self.fallback_system
                    result['fallback'] = True
                    return result
                except Exception as fe:
                    logger.error(f"Fallback system ({self.fallback_system}) also failed: {fe}")
            
            # All systems failed
            raise RuntimeError(f"All database systems unavailable: primary={e}, fallback={fe if self.fallback_system else 'none'}")
    
    async def _execute_on_system(self, system_name: str, query_type: str, query_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def test_integration(self) -> Dict[str, Any]:
        ...
        try:
            ...
            await self._health_check_all()
            available_count = sum(1 for config in self.systems.values() if config.available)
            
            test_results['system_tests'] = {
                'total_systems': len(self.systems),
                'available_systems': available_count,
                'systems_detail': {name: config.available for name, config in self.systems.items()}
            }
        ...
        logger.info(f"Integration test complete - Status: {test_results['bridge_status']}")
        return test_results
