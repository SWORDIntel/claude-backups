...
class PerformanceMonitor:
    ...
    def cache_response(self, request: AgentRequest, response: AgentResponse) -> None:
        ...
        if response.success and response.execution_time_ms < 1000:  # Only cache fast, successful responses
            cache_key = self.get_cache_key(request)
    ...
    def record_performance(self, response: AgentResponse) -> None:
        ...
        if self.metrics['requests_total'] > 0:
            self.metrics['success_rate'] = self.success_count / self.metrics['requests_total']
            self.metrics['cache_hit_rate'] = self.metrics['requests_cached'] / self.metrics['requests_total']
...
class AgentRegistryUnified:
    ...
    def __init__(self, config_manager: ConfigurationManager):
        ...
        self.agent_categories = {
            ...
            'language': ['c-internal', 'python-internal', 'rust-internal', 'go-internal'],
    ...
    async def discover_agents(self) -> int:
        ...
        if agents_dir.exists():
            for agent_file in agents_dir.glob('*.md'):
                ...
                if agent_name not in ['template', 'readme'] and agent_name not in self.agents:
    ...
    async def _extract_agent_metadata(self, agent_file: Path) -> Optional[AgentMetadata]:
    ...
    def get_similar_agents(self, agent_name: str, limit: int = 3) -> List[str]:
        ...
        similar = [
            name for name, metadata in self.agents.items()
...
class IntegrationPathManager:
    ...
    def _select_load_balanced_path(self) -> IntegrationPath:
        ...
        available_paths = [path for path, available in self.path_health.items() if available]
        if available_paths:
    ...
    async def execute_with_fallback(self, path: IntegrationPath, request: AgentRequest) -> AgentResponse:
...
class FallbackOrchestrator:
    ...
    def __init__(self, config_manager: ConfigurationManager, agent_registry: AgentRegistryUnified):
        self.config = config_manager
    ...
    async def handle_failure(self, error: Exception, request: AgentRequest, 
                           path_manager: IntegrationPathManager) -> AgentResponse:
        ...
        for strategy in strategies:
            try:
                ...
                logger.warning(f"Fallback strategy {strategy.value} failed: {fallback_error}")
                continue
    ...
    async def _retry_same_agent(self, request: AgentRequest, path_manager: IntegrationPathManager, 
                               error: Exception) -> AgentResponse:
        """Retry the same agent with different parameters"""
        if request.max_retries > 0:
            retry_request = request
            retry_request.max_retries -= 1
            retry_request.timeout = min(retry_request.timeout * 1.5, 60)  # Increase timeout
    ...
    async def _try_similar_agents(self, request: AgentRequest, path_manager: IntegrationPathManager,
                                 error: Exception) -> AgentResponse:
        ...
        for similar_agent in similar_agents:
            try:
                ...
                response = await path_manager.execute_with_fallback(path, similar_request)
    ...
    async def _use_general_purpose(self, request: AgentRequest, path_manager: IntegrationPathManager,
                                  error: Exception) -> AgentResponse:
    ...
    async def _emergency_mode(self, request: AgentRequest, path_manager: IntegrationPathManager,
                             error: Exception) -> AgentResponse:
...
This response ensures the system remains functional even during failures."""
    ...
    async def _direct_execution(self, request: AgentRequest, path_manager: IntegrationPathManager,
                               error: Exception) -> AgentResponse:
    ...
    async def _error_response(self, request: AgentRequest, path_manager: IntegrationPathManager,
                             error: Exception) -> AgentResponse:
...
class ClaudeCodeIntegrationHub:
    ...
    def __init__(self):
        ...
        self.fallback_orchestrator = FallbackOrchestrator(self.config_manager, self.agent_registry)
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            available_paths = sum(1 for available in self.path_manager.path_health.values() if available)
    ...
    async def route_agent_request(self, request: AgentRequest) -> AgentResponse:
        ...
        try:
            ...
            logger.debug(f"Selected path: {optimal_path.value} for {request.agent_name}")
            
            # Execute with fallback
            response = await self.path_manager.execute_with_fallback(optimal_path, request)
            ...
            response = await self.fallback_orchestrator.handle_failure(e, request, self.path_manager)
            self.performance_monitor.record_performance(response)
            return response
    
    async def invoke_agent(self, agent_name: str, prompt: str, **kwargs) -> AgentResponse:
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'cache_hit_rate': system_health.performance_metrics.get('cache_hit_rate', 0.0),
            'avg_response_time': system_health.performance_metrics.get('avg_response_time', 0.0),
...
async def main():
    ...
    print(f"Performance: {status['avg_response_time']:.1f}ms avg, {status['success_rate']*100:.1f}% success")
    print()
    ...
    if hub.get_available_agents():
        ...
        response = await hub.invoke_agent(test_agent, "Test integration hub functionality")
