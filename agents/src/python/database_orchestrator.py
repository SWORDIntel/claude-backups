...
from production_orchestrator import ProductionOrchestrator, CommandSet, CommandStep, ExecutionMode
import uuid
...
class DatabaseIntegratedOrchestrator(ProductionOrchestrator):
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            logger.info("✅ PostgreSQL 17 connection pool initialized (20-100 connections)")
            ...
            if recovered_workflows:
                logger.info(f"✅ Recovered {len(recovered_workflows)} interrupted workflows")
    ...
    async def execute_command_set(self, command_set: CommandSet, user_id: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            await self.store_workflow_completion(command_set.id, result, "completed", duration)
            
            # Log security event
            await self.log_security_event("workflow_completed", user_id or "system", 
                                         command_set.name, f"Duration: {duration:.2f}s")
            
            # Update metrics
            await self.update_database_metrics()
            
            logger.info(f"✅ Workflow '{command_set.name}' completed in {duration:.2f}s")
            return result
            
        except Exception as e:
            # Store failure with full context
            duration = time.time() - start_time
            await self.store_workflow_completion(command_set.id, str(e), "failed", duration)
            
            # Log security event
            await self.log_security_event("workflow_failed", user_id or "system",
                                         command_set.name, f"Error: {str(e)}")
    ...
    async def store_workflow_start(self, command_set: CommandSet, user_id: str):
        ...
        async with self.db_pool.acquire() as conn:
            ...
            """, 
            command_set.id, command_set.name, command_set.type.value, 
            command_set.mode.value, command_set.priority.value,
            json.dumps({"steps": [step.id for step in command_set.steps]}),
            user_id, command_set.timeout, len(command_set.steps))
            ...
            for i, step in enumerate(command_set.steps):
                ...
                """,
                step_id, command_set.id, step.agent, step.action,
                json.dumps(step.payload), step.can_fail, 
                command_set.dependencies.get(step.id, []))
    
    async def execute_workflow_with_persistence(self, command_set: CommandSet, user_id: str) -> Dict[str, Any]:
        ...
        for i, step in enumerate(command_set.steps):
            ...
            db_step_id = db_steps[i]['step_id'] if i < len(db_steps) else f"{command_set.id}_step_{i}_{str(uuid.uuid4())[:8]}"
            ...
            try:
                ...
                await self.store_step_result(db_step_id, result, "completed", step_duration)
                
                # Track agent performance
                await self.track_agent_performance(step.agent, step.action, 
                                                 step_duration * 1000, len(str(result)), 
                                                 True, command_set.id, user_id)
                
                results[db_step_id] = result
                logger.info(f"✅ Step '{step.agent}.{step.action}' completed in {step_duration:.2f}s")
                
            except Exception as e:
                step_duration = time.time() - step_start
                
                # Store step failure
                await self.store_step_result(db_step_id, str(e), "failed", step_duration)
                
                # Track agent performance (failure)
                await self.track_agent_performance(step.agent, step.action,
                                                 step_duration * 1000, len(str(e)),
                                                 False, command_set.id, user_id)
                
                if not step.can_fail:
                    await self.mark_workflow_failed(command_set.id, str(e))
                    raise
                    
                logger.warning(f"⚠️  Step '{step.agent}.{step.action}' failed but marked as optional")
                results[db_step_id] = {"error": str(e), "can_fail": True}
        
        return results
    
    async def simulate_agent_execution(self, step: CommandStep, user_id: str) -> Dict[str, Any]:
        ...
        await self.log_security_event("agent_invocation", user_id or "system",
                                     step.agent, f"Action: {step.action}")
        
        # Simulate processing time based on agent complexity
        processing_time = {
            'director': 2.0, 'architect': 3.0, 'security': 1.5,
            'constructor': 2.5, 'testbed': 1.0, 'docgen': 1.5
        }.get(step.agent, 1.0)
    ...
    async def track_agent_performance(self, agent_name: str, action: str, duration_ms: float,
                                    output_size: int, success: bool, workflow_id: str, user_id: str):
    ...
    async def store_step_result(self, step_id: str, result: Any, status: str, duration: float):
        ...
        async with self.db_pool.acquire() as conn:
            await conn.execute("""
                ...
                WHERE step_id = $1
            """, step_id, status, json.dumps(result) if result else None)
    
    async def store_workflow_completion(self, workflow_id: str, result: Any, 
    ...
    async def recover_workflows(self) -> List[CommandSet]:
        ...
        async with self.db_pool.acquire() as conn:
            ...
            for row in interrupted_workflows:
                ...
                from production_orchestrator import CommandType
    ...
    async def update_database_metrics(self):
        ...
        async with self.db_pool.acquire() as conn:
            ...
            self.db_metrics.update({
                ...
                "avg_workflow_duration": float(db_stats['avg_workflow_duration'] or 0),
        ...
        return CommandSet(
            ...
            steps=[
                CommandStep(
                    ...
                    payload={"database": "PostgreSQL 17", "integration": "Python orchestrator"}
                ),
                CommandStep(
                    agent="security", 
...
async def demo_database_integration():
    ...
    print(f"\n📈 Dashboard Metrics:")
    print(f"  Total Workflows: {dashboard.get('workflows', {}).get('total_workflows', 0)}")
    print(f"  Active Workflows: {dashboard.get('workflows', {}).get('active_workflows', 0)}")
    ...
    print("🗄️ All workflow state, metrics, and security events now persisted in PostgreSQL 17")
