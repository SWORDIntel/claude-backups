...
class DockerAgent:
    ...
    async def build_image(self, name: str, dockerfile_path: str, 
                         context_path: str = ".", optimize: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            image = DockerImage(
                ...
                size_mb=random.uniform(100, 500) if not optimize else random.uniform(50, 200),
                layers=random.randint(15, 25) if not optimize else random.randint(8, 15),
            ...
            logger.info(f"Image built: {name} ({image.size_mb:.1f}MB, {image.layers} layers)")
    ...
    async def run_container(self, config: ContainerConfig) -> Dict[str, Any]:
        ...
        try:
            ...
            container_id = hashlib.md5(f"{config.name}{datetime.now()}".encode()).hexdigest()[:12]
            ...
            logger.info(f"Container started: {config.name} ({container_id}) in {result['startup_time']:.3f}s")
    ...
    async def generate_compose_file(self, services: List[Dict[str, Any]], 
    ...
    async def _generate_service_config(self, service: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Add health check
        config['healthcheck'] = {
            'test': service.get('health_check', 'CMD curl -f http://localhost/health || exit 1'),
            'interval': '30s',
            'timeout': '10s',
            'retries': 3,
            'start_period': '40s'
    ...
    async def _generate_networks_config(self, services: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def _generate_volumes_config(self, services: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    async def create_service(self, service: SwarmService) -> Dict[str, Any]:
        ...
        try:
            ...
            # Generate service ID
            service_id = hashlib.md5(f"{service.name}{datetime.now()}".encode()).hexdigest()[:10]
    ...
    async def scale_service(self, service_name: str, replicas: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Service scaled: {service_name} to {replicas} replicas in {result['scale_time']:.2f}s")
    ...
    async def deploy_stack(self, stack_name: str, compose_file: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Stack deployed: {stack_name} ({result['services_deployed']} services) in {result['deploy_time']:.2f}s")
    ...
    async def scan_image_security(self, image_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Security scan complete: {image_name} (Score: {result['security_score']}/100)")
    ...
    async def optimize_dockerfile(self, dockerfile_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            result['estimated_size_reduction'] = random.uniform(60, 80)  # 60-80% reduction
            ...
            logger.info(f"Dockerfile optimized: {len(optimizations)} optimizations applied")
...
async def main():
    ...
    build_result = await agent.build_image(
        "my-app:v1.0",
        "Dockerfile",
        optimize=True
    )
    ...
    print(f"Replicas: {scale_result.get('old_replicas', 0)} → {scale_result.get('new_replicas', 0)}")
    ...
    print(f"Vulnerabilities: Critical: {vulnerabilities.get('critical', 0)}, High: {vulnerabilities.get('high', 0)}")
