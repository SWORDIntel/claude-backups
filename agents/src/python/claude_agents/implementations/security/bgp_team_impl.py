...
class BGPBlueTeam:
    ...
    def __init__(self):
        ...
        self.logger.info(f"BGP Blue Team v{self.version} initialized - Pure Defense Mode")
    ...
    async def create_roa(self, prefix: str, origin_as: int, 
                        max_length: Optional[int] = None) -> ROA:
    ...
    async def detect_attack(self, route: BGPRoute) -> Optional[BGPIncident]:
        ...
        # Determine if attack
        if rpki_status == ValidationStatus.INVALID or anomaly_score > 0.85 or predicted_attack:
            incident = BGPIncident(
                incident_id=hashlib.md5(f"{route.prefix}{datetime.now()}".encode()).hexdigest()[:8],
            ...
            self.logger.critical(f"ATTACK DETECTED: {incident.attack_type.value} on {route.prefix}")
            return incident
    ...
    async def instant_response(self, incident: BGPIncident) -> Dict[str, Any]:
        ...
        self.logger.info(f"Initiating instant response to incident {incident.incident_id}")
...
class BGPRedTeam:
    ...
    async def deploy_attack_infrastructure(self) -> Dict[str, Any]:
        ...
        self.logger.info(f"Attack infrastructure deployed - {len(self.attack_speakers)} speakers ready")
        return result
    
    async def execute_prefix_hijack(self, target_prefix: str, 
                                   target_as: int) -> BGPIncident:
        ...
        incident = BGPIncident(
            incident_id=hashlib.md5(f"attack-{target_prefix}{datetime.now()}".encode()).hexdigest()[:8],
            timestamp=datetime.now(),
            attack_type=AttackType.PREFIX_HIJACK,
        ...
        self.logger.info(f"Hijack complete - Global propagation in {propagation_time:.1f}s")
    ...
    async def execute_subprefix_attack(self, target_prefix: str) -> BGPIncident:
        ...
        self.logger.info("Sub-prefix attack executed - Defense requires more specific ROAs")
        return incident
    ...
    async def execute_rpki_bypass(self, target_prefix: str) -> Dict[str, Any]:
        ...
        if result['rpki_defeated']:
            self.logger.info(f"RPKI bypassed using {len(result['successful_bypasses'])} methods")
    ...
    async def execute_as_path_poisoning(self, target_as: int, 
                                       victim_as: int) -> Dict[str, Any]:
        """Manipulate AS paths to redirect traffic"""
        self.logger.info(f"Executing AS path poisoning: target={target_as}, victim={victim_as}")
        ...
        self.logger.info(f"AS path poisoning {'successful' if result['traffic_redirected'] else 'failed'}")
        return result
...
class BGPPurpleTeam:
    ...
    def __init__(self):
        ...
        self.logger.info(f"BGP Purple Team v{self.version} initialized - Combined Operations")
    ...
    async def initialize_teams(self):
        ...
        self.logger.info("Purple team ready - Both attack and defense capabilities online")
    
    async def execute_purple_exercise(self, target_prefix: str = "192.0.2.0/24") -> Dict[str, Any]:
        """Execute comprehensive purple team exercise"""
        self.logger.info(f"Starting purple team exercise on {target_prefix}")
        
        exercise = {
            'exercise_id': hashlib.md5(f"exercise-{datetime.now()}".encode()).hexdigest()[:8],
            'target': target_prefix,
            'phases': [],
            'detections': [],
            'response_times': [],
        ...
        successful_defenses = sum(1 for p in exercise['phases'] if p.get('defended', False))
        exercise['overall_score'] = (successful_defenses / len(exercise['phases'])) * 100
    ...
    async def _test_basic_hijack(self, target_prefix: str) -> Dict[str, Any]:
        ...
        # Red team attacks
        attack_incident = await self.red_team.execute_prefix_hijack(target_prefix, 65001)
    ...
    async def generate_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            'successful_defenses': sum(1 for p in latest['phases'] if p.get('defended', False)),
            'gaps_identified': [],
            'recommendations': []
        }
    ...
    def get_purple_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_exercise_score': self.exercise_results[-1]['overall_score'] if self.exercise_results else None,
...
async def main():
    ...
    print("\nExercise Report:")
    print(f"  Successful Defenses: {report['successful_defenses']}/{report['phases_tested']}")
    if report['gaps_identified']:
