...
try:
    from claude_agents.orchestration.production_orchestrator import ProductionOrchestrator, ExecutionMode, CommandSet, CommandStep
    from claude_agents.learning.postgresql_learning_system import UltimatePostgreSQLLearningSystem, AgentTaskExecution
        ...
        for key, sequences in self.failure_patterns.items():
            if len(sequences) >= 2:
                patterns["failure_indicators"].append({
                    "pattern": key,
                    "failure_rate": len(sequences) / (len(sequences) + len(self.success_patterns.get(key, [])))
                })
...
class AdaptiveLearningEngine:
    ...
    def select_agents(self, context: ExecutionContext, 
                     capabilities: List[AgentCapability],
                     historical_data: List[AgentTaskExecution]) -> List[str]:
    ...
    def _conservative_selection(self, context: ExecutionContext,
                               capabilities: List[AgentCapability],
                               historical_data: List[AgentTaskExecution]) -> List[str]:
        ...
        for cap in capabilities:
            if (context.task_type in cap.task_types and
                cap.success_rate > 0.9 and
                cap.complexity_range[0] <= context.complexity <= cap.complexity_range[1]):
    ...
    def _experimental_selection(self, context: ExecutionContext,
                               capabilities: List[AgentCapability],
                               historical_data: List[AgentTaskExecution]) -> List[str]:
        ...
        potential_agents = [
            cap.agent_name for cap in capabilities
            if context.task_type in cap.task_types or 
               context.complexity >= cap.complexity_range[0]
        ]
    ...
    def _balanced_selection(self, context: ExecutionContext,
                           capabilities: List[AgentCapability],
                           historical_data: List[AgentTaskExecution]) -> List[str]:
    ...
    def _adaptive_selection(self, context: ExecutionContext,
                          capabilities: List[AgentCapability],
                          historical_data: List[AgentTaskExecution]) -> List[str]:
        ...
        matching_patterns = [
            p for p in patterns["common_sequences"]
        ...
        # Check task urgency
        if context.priority > 3 or (context.deadline and 
                                   context.deadline - datetime.now() < timedelta(hours=1)):
...
class EnhancedLearningOrchestrator:
    ...
    async def _get_intelligent_recommendations(self, 
        ...
        if self.learning_system and self.learning_enabled:
            ...
            pattern_agents = self.learning_engine.select_agents(
                context, 
                list(self.agent_capabilities.values()),
                historical_data
            )
            ...
            logger.info(f"Intelligent recommendations for {context.task_type}: {recommended_agents}")
        
        return recommended_agents
    
    async def _create_adaptive_command_set(self,
                                         context: ExecutionContext,
                                         recommended_agents: List[str]) -> CommandSet:
    ...
    def _generate_step_parameters(self, agent: str, 
                                 context: ExecutionContext, 
    ...
    def _determine_agent_action(self, agent: str, context: ExecutionContext) -> str:
        ...
        if agent_upper in action_mapping:
            return action_mapping[agent_upper].get(
                context.task_type, 
                action_mapping[agent_upper]["default"]
            )
    ...
    def _determine_parallelism(self, context: ExecutionContext, 
                              steps: List[CommandStep]) -> int:
    ...
    def _calculate_total_timeout(self, context: ExecutionContext,
                                steps: List[CommandStep]) -> int:
    ...
    async def _execute_with_monitoring(self, command_set: CommandSet,
                                      context: ExecutionContext) -> Dict[str, Any]:
    ...
    async def _monitor_execution(self, command_set: CommandSet,
                                context: ExecutionContext) -> Dict[str, Any]:
        ...
        monitoring_data["total_duration"] = monitoring_data["end_time"] - monitoring_data["start_time"]
    ...
    async def _record_and_learn(self, context: ExecutionContext,
                               result: Dict[str, Any],
    ...
    async def _maybe_trigger_learning(self):
        ...
        if any(triggers.values()):
            logger.info(f"Triggering learning cycle: {[k for k, v in triggers.items() if v]}")
    ...
    def _calculate_agent_capability(self, agent_name: str,
                                   performances: List[Dict]) -> AgentCapability:
    ...
    async def _generate_learning_report(self, metrics: LearningMetrics):
        ...
        report = {
            ...
            "top_agents": sorted(
                metrics.agent_efficiency.items(),
                key=lambda x: x[1],
                reverse=True
            )[:5],
            "recent_insights": [
    ...
    def _generate_recommendations(self, metrics: LearningMetrics) -> List[str]:
        ...
        if metrics.success_rate < 0.7:
            recommendations.append("Consider increasing exploration rate to find better patterns")
        ...
        if metrics.resource_utilization > 0.8:
            recommendations.append("Consider scaling resources or optimizing agent efficiency")
        
        if metrics.exploration_rate < 0.1:
            recommendations.append("Increase exploration to discover new effective patterns")
    ...
    def _check_execution_cache(self, context: ExecutionContext) -> Optional[Dict[str, Any]]:
    ...
    def _update_execution_cache(self, context: ExecutionContext, result: Dict[str, Any]):
...
if __name__ == "__main__":
    async def demo():
        ...
        for context in tasks:
            ...
            print(f"Type: {context.task_type}, Complexity: {context.complexity}, Priority: {context.priority}")
