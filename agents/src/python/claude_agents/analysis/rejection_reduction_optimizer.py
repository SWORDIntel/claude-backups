...
from claude_rejection_reducer import ClaudeRejectionReducer, StrategyResult, StrategyConfig
...
class RejectionReductionOptimizer:
    ...
    def _create_test_scenarios(self) -> List[Dict[str, Any]]:
        ...
        scenarios = [
            ...
            {
                'name': 'large_file',
                'content': '\n'.join([f'def function_{i}(): return {i}' for i in range(2000)]),
            ...
            },
            {
                ...
                'expected_triggers': ['password', 'exploit', 'payload', 'api_key', 'rm -rf']
            }
        ]
    ...
    async def optimize_parameters(self, iterations: int = 50) -> Dict[str, Any]:
        ...
        logger.info(f"Target: {self.target.min_acceptance_rate:.1%}-{self.target.max_acceptance_rate:.1%} acceptance rate")
        ...
        for iteration in range(iterations):
            ...
            if (self.target.min_acceptance_rate <= metrics.acceptance_rate <= self.target.max_acceptance_rate and
    ...
    async def _test_configuration(self, config: Dict[str, StrategyConfig]) -> PerformanceMetrics:
        ...
        for scenario in self.test_scenarios:
            try:
                ...
                for strategy_name, usage_count in reducer.stats.get('strategy_usage', {}).items():
        ...
        metrics = PerformanceMetrics(
            ...
            compression_ratio=statistics.mean(compression_ratios) if compression_ratios else 1.0,
    ...
    def _calculate_fitness_score(self, metrics: PerformanceMetrics) -> float:
        ...
        if self.target.min_acceptance_rate <= metrics.acceptance_rate <= self.target.max_acceptance_rate:
            acceptance_score = 1.0
        elif metrics.acceptance_rate > self.target.max_acceptance_rate:
            # Penalize over-optimization
            acceptance_score = max(0, 1.0 - (metrics.acceptance_rate - self.target.max_acceptance_rate) * 2)
        else:
        ...
        if metrics.average_processing_time <= self.target.max_processing_time:
            time_score = 1.0 - (metrics.average_processing_time / self.target.max_processing_time) * 0.5
        else:
            time_score = max(0, 1.0 - (metrics.average_processing_time / self.target.max_processing_time))
        ...
        if metrics.compression_ratio >= self.target.min_compression_ratio:
            ...
            compression_score = metrics.compression_ratio / self.target.min_compression_ratio
    ...
    def _generate_optimization_report(self) -> Dict[str, Any]:
        ...
        acceptance_rates = [entry['metrics'].acceptance_rate for entry in self.optimization_history]
        processing_times = [entry['metrics'].average_processing_time for entry in self.optimization_history]
        ...
        report = {
            'optimization_summary': {
                ...
                'target_achieved': (
                    self.target.min_acceptance_rate <= self.best_metrics.acceptance_rate <= self.target.max_acceptance_rate
                ),
            ...
            'optimization_progress': {
                ...
                'stability_metric': statistics.stdev(scores[-10:]) if len(scores) >= 10 else None
            },
    ...
    async def benchmark_current_system(self) -> Dict[str, Any]:
        ...
        benchmark = {
            ...
            'optimization_needed': baseline_metrics.acceptance_rate < self.target.min_acceptance_rate,
...
async def run_optimization(iterations: int = 50, 
                         target: OptimizationTarget = None) -> Dict[str, Any]:
    """
    ...
    print(f"Current acceptance rate: {benchmark['baseline_performance']['acceptance_rate']}")
    print(f"Target range: {benchmark['target_metrics']['min_acceptance_rate']}-{benchmark['target_metrics']['max_acceptance_rate']}")
    ...
    if benchmark['optimization_needed'] or iterations > 0:
        ...
        print(f"Best acceptance rate: {report['performance_metrics']['acceptance_rate']}")
        print(f"Processing time: {report['performance_metrics']['average_processing_time']}")
    ...
    async def main():
        ...
        if result['optimization_report']:
            ...
            with open(config_file, 'w') as f:
                json.dump(result['optimization_report']['production_config'], f, indent=2)
