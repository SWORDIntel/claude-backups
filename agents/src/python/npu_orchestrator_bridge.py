...
try:
    from production_orchestrator import (
        ...
        Priority, CommandType, HardwareAffinity, StandardWorkflows
    )
    from npu_accelerated_orchestrator import (
        NPUAcceleratedOrchestrator, NPUMode, NPUDevice
    )
...
class NPUOrchestratorBridge:
    ...
    def __init__(self, npu_mode: NPUMode = NPUMode.ADAPTIVE, enable_fallback: bool = True):
    ...
    async def execute_command_set(self, command_set: CommandSet) -> Dict[str, Any]:
        ...
        try:
            ...
            result['bridge_metrics'] = {
                ...
                'performance_gain': await self._calculate_performance_gain(execution_time, result['execution_method'])
            }
    ...
    async def _should_use_npu(self, command_set: CommandSet) -> bool:
        ...
        if (command_set.type in npu_beneficial_types and
            len(command_set.steps) >= 3):
            return True

        # NPU beneficial for agent selection tasks
        if any('select' in step.action.lower() or 'choose' in step.action.lower()
    ...
    def _command_set_to_description(self, command_set: CommandSet) -> str:
        ...
        for i, step in enumerate(command_set.steps, 1):
            ...
            if step.params:
                key_params = ', '.join(f"{k}={v}" for k, v in list(step.params.items())[:3])
                description += f" ({key_params})"
            description += "\n"

        return description

    async def _calculate_performance_gain(self, execution_time: float, method: str) -> float:
    ...
    async def invoke_agent(self, agent_name: str, action: str, params: Dict = None) -> Any:
        ...
        if (self.npu_available and
            any(keyword in action.lower() for keyword in ['analyze', 'predict', 'optimize', 'select'])):

            # Use NPU for intelligent operations
            workflow_desc = f"Execute {action} using {agent_name} agent"
            result = await self.npu_orchestrator.execute_intelligent_workflow(workflow_desc, params)
            return result
        else:
    ...
    async def _adaptive_optimization(self):
        ...
        if error_rate > self.fallback_error_threshold:
            logger.warning(f"High NPU error rate ({error_rate:.1%}), disabling NPU temporarily")
            self.npu_available = False
...
async def execute_with_npu_acceleration(workflow_description: str,
...
__all__ = [
    'NPUOrchestratorBridge',
    'get_npu_bridge',
    'execute_with_npu_acceleration'
]
