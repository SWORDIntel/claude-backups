...
if is_npu_task "$@" && [[ -x "$NPU_BRIDGE_SERVER" ]]; then
    echo "🧠 Routing to NPU Bridge for neural processing..."
...
def main():
    ...
    if args.setup:
        print("🔧 Setting up NPU integration...")
        ...
        if results.get("error"):
            ...
            print("✅ NPU integration setup completed")

            if results["npu_detected"]:
                print("🧠 Intel NPU detected")

            if results["wrapper_created"]:
                print(f"📝 NPU installer wrapper: {results.get('wrapper_path')}")

            if results["enhanced_wrapper_created"]:
                print(f"🚀 Enhanced Claude wrapper: {results.get('enhanced_wrapper_path')}")

    elif args.install_npu:
        print("📦 Installing NPU bridge...")
        success = integrator.install_npu_bridge_now()

        if success:
            print("✅ NPU bridge installed successfully")
        else:
            print("❌ NPU bridge installation failed")
            sys.exit(1)

    elif args.status:
        report = integrator.generate_integration_report()
        print("📊 NPU Integration Status:")
        print(f"   Claude Directory: {report['claude_dir']}")
        print(f"   NPU Installed: {'✅' if report['npu_installed'] else '❌'}")
        print(f"   Wrapper Available: {'✅' if report['wrapper_exists'] else '❌'}")
        print(f"   Enhanced Wrapper: {'✅' if report['enhanced_wrapper_exists'] else '❌'}")
