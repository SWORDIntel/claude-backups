...
class AdaptiveLRUCache:
    ...
    def _adapt_capacity(self):
        ...
        if current_hit_rate < self.target_hit_rate and self.capacity < self.max_capacity:
            # Increase capacity if hit rate is below target
            new_capacity = min(int(self.capacity * 1.2), self.max_capacity)
            logger.info(f"L1 Cache: Increasing capacity from {self.capacity} to {new_capacity} (hit rate: {current_hit_rate:.1f}%)")
            ...
            logger.info(f"L1 Cache: Decreasing capacity from {self.capacity} to {new_capacity} (hit rate: {current_hit_rate:.1f}%)")
            self.capacity = new_capacity
            self._evict_lru()
    ...
    def put(self, key: str, value: Any, ttl_seconds: Optional[int] = None, metadata: Dict = None):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self.lock:
            return {
                ...
                'avg_size_per_entry': self.metrics.total_size_bytes / max(len(self.cache), 1),
...
class DistributedRedisCache:
    ...
    def __init__(self, redis_url: str = "redis://localhost:6379/0", key_prefix: str = "claude_cache"):
    ...
    async def warm_cache(self, warming_data: Dict[str, Any]):
        ...
        for key, value in warming_data.items():
            if await self.put(key, value, ttl_seconds=7200):  # 2 hour TTL for warmed data
                warmed_count += 1
...
class PostgreSQLMaterializedViewCache:
    ...
    def __init__(self, database_url: str = "postgresql://claude_agent:password@localhost:5433/claude_agents_auth"):
    ...
    async def put(self, key: str, value: Any, ttl_seconds: Optional[int] = None):
        ...
        try:
            ...
            async with self.pool.acquire() as conn:
                ...
                """, key, json.dumps(json_value, default=str), ttl_seconds, size_bytes)
    ...
    async def invalidate(self, key: str) -> bool:
        ...
        try:
            async with self.pool.acquire() as conn:
                result = await conn.execute("DELETE FROM cache_entries WHERE cache_key = $1", key)
                if result == "DELETE 1":
                    self.metrics.invalidation_count += 1
                    return True
                return False
    ...
    async def create_materialized_view(self, view_name: str, query: str, refresh_interval_hours: int = 1):
        ...
        try:
            async with self.pool.acquire() as conn:
                ...
                await conn.execute(f"CREATE UNIQUE INDEX IF NOT EXISTS idx_{view_name}_refresh ON {view_name} (1)")
                ...
                async with self.pool.acquire() as conn:
                    ...
                    if cache_stats:
                        stats.update({
                            ...
                            'total_size_bytes': cache_stats['total_size_bytes'] or 0,
                            'avg_access_count': float(cache_stats['avg_access_count'] or 0),
...
class MultiLevelCacheManager:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.l3_cache = PostgreSQLMaterializedViewCache(
            database_url=self.config.get('postgres_url', 'postgresql://claude_agent:password@localhost:5433/claude_agents_auth')
        )
        
        # Combined metrics
        self.combined_metrics = CacheMetrics(CacheLevel.MISS)  # Will track overall performance
        
        # Cache promotion/demotion thresholds
        self.promotion_threshold = self.config.get('promotion_threshold', 3)  # Promote after 3 accesses
        self.demotion_age_hours = self.config.get('demotion_age_hours', 24)  # Demote after 24 hours
    ...
    def _init_prometheus_metrics(self):
        ...
        self.prometheus_metrics = {
            ...
            'cache_access_time': Histogram('cache_access_seconds', 'Cache access time', ['level']),
            'cache_size': Gauge('cache_size_bytes', 'Cache size in bytes', ['level']),
            'cache_entries': Gauge('cache_entries_total', 'Total cache entries', ['level'])
        }
    ...
    async def initialize(self):
        ...
        if PROMETHEUS_AVAILABLE and self.config.get('prometheus_port'):
            ...
            logger.info(f"Prometheus metrics available on port {self.config['prometheus_port']}")
        
        logger.info(f"Multi-Level Cache initialized - L2: {l2_success}, L3: {l3_success}")
        return True
    ...
    async def get(self, key: str, default: Any = None) -> Any:
        ...
        if PROMETHEUS_AVAILABLE and 'cache_access_time' in self.prometheus_metrics:
            self.prometheus_metrics['cache_access_time'].labels(level='MISS').observe(access_time / 1000)
        
        return default
    
    async def put(self, key: str, value: Any, ttl_seconds: Optional[int] = None, cache_level: str = "ALL"):
        """
    ...
    def _update_prometheus_size(self):
        ...
        if 'cache_size' in self.prometheus_metrics:
            l1_stats = self.l1_cache.get_stats()
            self.prometheus_metrics['cache_size'].labels(level='L1').set(l1_stats['total_size_bytes'])
            
        if 'cache_entries' in self.prometheus_metrics:
            l1_stats = self.l1_cache.get_stats()
            self.prometheus_metrics['cache_entries'].labels(level='L1').set(l1_stats['size'])
...
def cache_result(cache_manager: MultiLevelCacheManager, ttl_seconds: int = 3600, 
                 cache_level: str = "ALL", key_prefix: str = ""):
    """
...
async def main():
    ...
    # Test data
    test_data = {
        f"test_key_{i}": f"test_value_{i}" * 100 for i in range(1000)
    }
