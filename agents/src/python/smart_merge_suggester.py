...
class SmartMergeSuggester:
    ...
    async def _load_merge_patterns(self):
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                logger.info(f"Loaded {len(self.strategy_patterns)} strategy patterns, "
                           f"{len(self.branch_patterns)} branch patterns")
                
        except Exception as e:
            logger.warning(f"Could not load merge patterns: {e}")
    
    def _analyze_branch_divergence(self, source_branch: str, target_branch: str) -> MergeAnalysis:
        ...
        try:
            ...
            lines_added = sum(item.stats.total_lines_added for item in source_diff if hasattr(item, 'stats'))
            lines_deleted = sum(item.stats.total_lines_deleted for item in source_diff if hasattr(item, 'stats'))
            ...
            branch_age = (datetime.now() - datetime.fromtimestamp(source_commit.committed_date)).days
            
            # Merge base distance
            merge_base_distance = len(list(self.repo.iter_commits(f'{base_commit.hexsha}..{source_branch}')))
    ...
    def _evaluate_fast_forward_strategy(self, analysis: MergeAnalysis) -> MergeStrategy:
        ...
        return MergeStrategy(
            ...
            cons=[
                "Only works when target is ancestor of source",
                "Loses merge context"
            ] if confidence > 0.5 else ["Not applicable for current branch state"],
            prerequisites=[
    ...
    def _evaluate_no_ff_strategy(self, analysis: MergeAnalysis) -> MergeStrategy:
        ...
        return MergeStrategy(
            ...
            prerequisites=[
                "Resolve any conflicts first" if analysis.has_conflicts else "No major prerequisites"
            ],
            command_sequence=[
                f"git checkout target_branch",
    ...
    def _evaluate_squash_strategy(self, analysis: MergeAnalysis) -> MergeStrategy:
        ...
        return MergeStrategy(
            ...
            cons=[
                ...
                "Requires manual commit message"
            ],
            prerequisites=[
    ...
    async def _get_conflict_predictions(self, source_branch: str, target_branch: str) -> Dict[str, Any]:
    ...
    async def suggest_merge_strategy(self, source_branch: str, target_branch: str) -> MergeRecommendation:
        """Generate comprehensive merge strategy recommendation"""
        try:
            logger.info(f"Analyzing merge strategy for {source_branch} -> {target_branch}")
            
            # Analyze branch divergence
            analysis = self._analyze_branch_divergence(source_branch, target_branch)
            
            # Get conflict predictions
            conflict_data = await self._get_conflict_predictions(source_branch, target_branch)
            if conflict_data['success']:
                analysis.conflict_count = conflict_data.get('total_conflicts', analysis.conflict_count)
            ...
            merge_message = self._generate_merge_message(source_branch, target_branch, analysis)
            
            # Create checklists
            pre_merge_checklist = self._create_pre_merge_checklist(analysis, primary_strategy)
            post_merge_checklist = self._create_post_merge_checklist(analysis, primary_strategy)
            ...
            await self._store_recommendation(source_branch, target_branch, primary_strategy, analysis)
            ...
            logger.info(f"Recommended strategy: {primary_strategy.strategy_type} "
    ...
    def _generate_merge_message(self, source_branch: str, target_branch: str, 
                               analysis: MergeAnalysis) -> str:
        """Generate intelligent merge commit message"""
        try:
            # Get recent commits from source branch
            commits = list(self.repo.iter_commits(f'{target_branch}..{source_branch}', max_count=10))
            ...
            for msg in commit_messages:
                for conv_type in conventional_types:
                    if msg.lower().startswith(f'{conv_type}:') or msg.lower().startswith(f'{conv_type}('):
            ...
            if analysis.commits_ahead == 1:
                ...
                if len(analysis.authors_involved) > 1:
                    summary_parts.append(f"{len(analysis.authors_involved)} contributors")
    ...
    def _create_pre_merge_checklist(self, analysis: MergeAnalysis, 
        ...
        if analysis.has_conflicts:
            checklist.append("✓ Review predicted conflicts and prepare resolution strategy")
        ...
        if strategy.strategy_type == 'rebase':
            ...
            checklist.append("✓ Prepare comprehensive commit message for squashed commits")
            checklist.append("✓ Review individual commits to ensure nothing important is lost")
    ...
    def _create_post_merge_checklist(self, analysis: MergeAnalysis, 
                                    strategy: MergeStrategy) -> List[str]:
    ...
    def _assess_risk_level(self, analysis: MergeAnalysis, strategy: MergeStrategy) -> str:
    ...
    def _create_rollback_plan(self, target_branch: str, strategy: MergeStrategy) -> str:
        ...
        return f"{base_plan}. Always verify with 'git log' before executing rollback. " \
               f"Consider creating a backup branch before merge for safety."
    
    async def _store_recommendation(self, source_branch: str, target_branch: str, 
                                   strategy: MergeStrategy, analysis: MergeAnalysis):
    ...
    async def record_merge_outcome(self, source_branch: str, target_branch: str,
                                 strategy_used: str, success: bool, 
                                 actual_conflicts: int = 0, merge_time_minutes: int = 0):
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                if rec:
                    ...
                    accuracy = 1.0 if (success and strategy_match) else 0.5 if success else 0.0
                    ...
                    if self.total_recommendations > 0:
                        self.recommendation_accuracy = self.successful_merges / self.total_recommendations
                    
                    logger.info(f"Recorded merge outcome: {strategy_used}, success={success}, "
    ...
    async def get_suggester_metrics(self) -> Dict[str, Any]:
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                return {
                    'total_recommendations': overall_stats['total_recommendations'] or 0,
                    'completed_merges': overall_stats['completed_merges'] or 0,
                    'success_rate': (overall_stats['successful_merges'] or 0) / max(overall_stats['completed_merges'] or 1, 1),
                    ...
                    'strategy_performance': [
                        {
                            ...
                            'success_rate': stat['successes'] / stat['recommendations'] if stat['recommendations'] > 0 else 0.0,
...
class SmartMergeSuggesterAPI:
    ...
    async def suggest_merge(self, source_branch: str, target_branch: str) -> Dict[str, Any]:
        """API endpoint for merge suggestions"""
        if not self._initialized:
            await self.initialize()
        
        try:
            recommendation = await self.suggester.suggest_merge_strategy(source_branch, target_branch)
    ...
    async def record_outcome(self, source_branch: str, target_branch: str,
                           strategy_used: str, success: bool, 
                           actual_conflicts: int = 0, merge_time_minutes: int = 0) -> Dict[str, Any]:
        ...
        try:
            await self.suggester.record_merge_outcome(
                source_branch, target_branch, strategy_used, 
                success, actual_conflicts, merge_time_minutes
            )
            return {'success': True}
        except Exception as e:
            return {'success': False, 'error': str(e)}
...
async def main():
    ...
    try:
        repo_path = "/home/john/claude-backups"
        ...
        print(f"Conflict predictor available: {metrics.get('conflict_predictor_available', False)}")
