-- Git Intelligence Database Schema
-- PostgreSQL 16+ with pgvector extension for neural embeddings
-- Team Echo Implementation

-- Enable pgvector extension for neural embeddings
...
CREATE INDEX IF NOT EXISTS idx_conflict_features_vector ON git_intelligence.conflict_features USING ivfflat (feature_vector vector_cosine_ops);
...
CREATE INDEX IF NOT EXISTS idx_code_embeddings_similarity ON git_intelligence.code_embeddings USING ivfflat (embedding vector_cosine_ops);
...
CREATE INDEX IF NOT EXISTS idx_pattern_embeddings_similarity ON git_intelligence.code_pattern_embeddings USING ivfflat (embedding vector_cosine_ops);
...
-- Note: In production, these would be scheduled via pg_cron or external scheduler
...
-- GRANT USAGE ON SCHEMA git_intelligence TO git_intelligence_user;
-- GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA git_intelligence TO git_intelligence_user;
-- GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA git_intelligence TO git_intelligence_user;