...
class SmartTruncator:
    ...
    def truncate(text: str, max_tokens: int = 500) -> str:
        ...
        for i, word in enumerate(middle_section):
            if any(key in word.lower() for key in 
                   ['error', 'warning', 'failed', 'success', 'result', 'complete']):
                # Include context around important words
                important_words.extend(middle_section[max(0, i-5):min(len(middle_section), i+5)])
