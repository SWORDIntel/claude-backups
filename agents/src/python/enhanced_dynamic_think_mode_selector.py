...
from dynamic_think_mode_selector import (
    ComplexityFeatures, ThinkModeAnalysis, ThinkModeDecision, TaskComplexity,
    NpuComplexityAnalyzer, CpuComplexityAnalyzer, DynamicThinkModeSelector
)

# Import auto-calibration components
from think_mode_auto_calibration import (
    ThinkModeAutoCalibrator, CalibratedComplexityAnalyzer,
    FeedbackType, WeightConfiguration
)
...
class EnhancedDynamicThinkModeSelector(DynamicThinkModeSelector):
    ...
    def __init__(self, enable_auto_calibration: bool = True, docker_integration: bool = True):
        ...
        if self.enable_auto_calibration:
            ...
            self.calibrated_analyzer = CalibratedComplexityAnalyzer(self.auto_calibrator)
    ...
    async def initialize_system(self) -> bool:
        ...
        try:
            ...
            if self.docker_orchestrator:
                deployment_success = await self.docker_orchestrator.deploy_calibration_system()
                if deployment_success:
                    self.logger.info("Docker calibration system deployed")
                else:
                    self.logger.warning("Docker deployment failed, continuing without Docker integration")
                    self.docker_orchestrator = None
    ...
    async def analyze_task_complexity_enhanced(self,
                                             task_text: str,
                                             context: Dict[str, Any] = None,
                                             collect_feedback: bool = True) -> EnhancedAnalysisResult:
        """Enhanced complexity analysis with auto-calibration"""
        start_time = time.time()
        self.logger.info(f"Enhanced analysis for {len(task_text)} characters (calibration: {self.enable_auto_calibration})")

        try:
            # Determine which analyzer to use
            if self.enable_auto_calibration and self.calibrated_analyzer:
                # Use calibrated analyzer
                complexity_score, npu_used = self.calibrated_analyzer.analyze_complexity(task_text)
            ...
            base_analysis = ThinkModeAnalysis(
                decision=ThinkModeDecision.INTERLEAVED if complexity_score >= self.config['complexity_threshold'] else ThinkModeDecision.NO_THINKING,
                complexity_score=complexity_score,
                confidence=min(complexity_score * 1.2, 1.0) if complexity_score >= self.config['complexity_threshold'] else 1.0 - complexity_score,
                reasoning=self._generate_enhanced_reasoning(complexity_score, features, calibration_used, weight_version),
                processing_time_ms=(time.time() - start_time) * 1000,
                npu_accelerated=npu_used
            )

            # Add agent recommendations using enhanced analysis
            base_analysis.agent_recommendations = self._analyze_enhanced_agent_coordination(task_text, features, complexity_score)
            ...
            enhanced_result = EnhancedAnalysisResult(
                ...
                improvement_suggestions=self._generate_improvement_suggestions(features, complexity_score)
            )

            # Collect feedback for learning if enabled
            if collect_feedback and self.auto_calibrator:
                feedback_id = await self._collect_learning_feedback(task_text, enhanced_result, features)
            ...
            self.enhanced_metrics['avg_calibration_time'] = (
                (self.enhanced_metrics['avg_calibration_time'] * (self.enhanced_metrics['total_enhanced_analyses'] - 1) + calibration_time) /
                self.enhanced_metrics['total_enhanced_analyses']
            )

            self.logger.info(f"Enhanced analysis complete: {base_analysis.decision.value} "
    ...
    def _calculate_calibration_confidence(self) -> float:
        ...
        sample_confidence = min(feedback_count / 100.0, 1.0)  # Max confidence at 100+ samples
    ...
    def _generate_enhanced_reasoning(self,
        ...
        if features.word_count > 100:
            reasoning_parts.append(f"High word count ({features.word_count}) indicates complex task")

        if features.technical_terms > 5:
            reasoning_parts.append(f"Technical complexity ({features.technical_terms} technical terms)")

        if features.multi_step_indicators > 3:
            reasoning_parts.append(f"Multi-step process ({features.multi_step_indicators} indicators)")
    ...
    def _analyze_enhanced_agent_coordination(self,
                                           task_text: str,
                                           features: ComplexityFeatures,
        ...
        if self.auto_calibrator:
            ...
            if calibration_metrics.get('performance', {}).get('current_accuracy', 0) < 0.8:
    ...
    def _generate_improvement_suggestions(self,
        ...
        if complexity_score < 0.3 and features.word_count < 20:
            suggestions.append("Consider providing more context for better complexity assessment")

        if features.technical_terms > 0 and features.documentation_needed:
            suggestions.append("Technical terms detected - documentation agent recommended")

        if features.multi_step_indicators > 0 and 'director' not in self._analyze_agent_coordination_needs(""):
            suggestions.append("Multi-step process detected - strategic coordination recommended")

        if features.agent_coordination_needed and complexity_score < 0.5:
            suggestions.append("Task complexity may be underestimated for multi-agent coordination")

        if complexity_score > 0.8:
            suggestions.append("High complexity task - consider breaking into smaller subtasks")
    ...
    async def _collect_learning_feedback(self,
                                       task_text: str,
                                       enhanced_result: EnhancedAnalysisResult,
                                       features: ComplexityFeatures) -> str:
        ...
        try:
            ...
            if optimization:
                ...
                if optimization.recommended_deployment:
                    deployed = await self.auto_calibrator.deploy_optimized_weights(optimization)
    ...
    async def get_enhanced_system_status(self) -> Dict[str, Any]:
        ...
        if self.auto_calibrator:
            enhanced_status['calibration_status'] = self.auto_calibrator.get_calibration_metrics()

        # Add Docker status
        if self.docker_orchestrator:
            enhanced_status['docker_status'] = await self.docker_orchestrator.get_system_status()
...
async def main():
    ...
    try:
        ...
        if await selector.initialize_system():
            ...
            print("⚠️  System initialization had issues, continuing with limited functionality")
        ...
        for i, task in enumerate(test_cases, 1):
            ...
            print(f"   Calibrated: {'Yes' if enhanced_result.calibration_used else 'No'} (v{enhanced_result.weight_version})")
            ...
            if enhanced_result.improvement_suggestions:
                print(f"   Suggestions: {'; '.join(enhanced_result.improvement_suggestions)}")
        ...
        if 'error' not in optimization_result:
            ...
            print(f"   Improvement: {optimization_result.get('improvement_score', 0):.3f}")
        else:
        ...
        print(f"   Total Enhanced Analyses: {status['enhanced_metrics']['total_enhanced_analyses']}")
        print(f"   Calibration Improvements: {status['enhanced_metrics']['calibration_improvements']}")
        print(f"   Learning Feedback: {status['enhanced_metrics']['learning_feedback_collected']}")
        print(f"   Weight Optimizations: {status['enhanced_metrics']['weight_optimizations_performed']}")
        print(f"   Avg Calibration Time: {status['enhanced_metrics']['avg_calibration_time']:.1f}ms")
