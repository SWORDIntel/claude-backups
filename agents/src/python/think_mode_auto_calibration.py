...
from dynamic_think_mode_selector import (
    ComplexityFeatures, ThinkModeAnalysis, ThinkModeDecision,
    NpuComplexityAnalyzer, CpuComplexityAnalyzer
)
...
class WeightConfiguration:
    ...
    length_bonus_thresholds: List[Tuple[int, float]] = field(default_factory=lambda: [(20, 0.1), (50, 0.2), (100, 0.3)])
    version: int = 1
...
class ThinkModeAutoCalibrator:
    ...
    def _initialize_ml_models(self):
        ...
        if not ML_AVAILABLE:
            self.logger.warning("ML libraries not available - using simple optimization")
            return
    ...
    async def record_decision_feedback(self,
                                     task_text: str,
        ...
        try:
            ...
            self.logger.debug(f"Recorded feedback for task {task_hash[:8]} "
    ...
    async def _store_feedback_in_database(self, task_text: str, feedback: FeedbackRecord):
    ...
    def _update_accuracy_metrics(self):
        ...
        if total_with_feedback > 0:
            ...
            self.logger.debug(f"Current accuracy: {self.metrics['current_accuracy']:.3f} "
                            f"({correct_decisions}/{total_with_feedback})")

    async def optimize_weights(self, strategy: CalibrationStrategy = CalibrationStrategy.ENSEMBLE_VOTING) -> OptimizationResult:
        ...
        try:
            ...
            if len(training_data['features']) < self.calibration_config['min_feedback_samples']:
                self.logger.warning(f"Insufficient training data: {len(training_data['features'])} samples "
            ...
            self.metrics['avg_optimization_time'] = (
                (self.metrics['avg_optimization_time'] * self.metrics['calibrations_performed'] + optimization_time) /
                (self.metrics['calibrations_performed'] + 1)
            )
            self.metrics['calibrations_performed'] += 1

            self.logger.info(f"Weight optimization completed in {optimization_time:.1f}ms "
    ...
    async def _prepare_training_data(self) -> Dict[str, Any]:
        ...
        for feedback in feedback_with_actuals:
            ...
            if feedback.actual_complexity is not None:
                ...
                complexity_targets.append(feedback.predicted_complexity)  # Use prediction as fallback
    ...
    async def _ensemble_weight_optimization(self, training_data: Dict[str, Any]) -> OptimizationResult:
        ...
        new_weights = WeightConfiguration(
            word_count_weight=max(0.001, current_weights.word_count_weight * (1 + feature_importance[0] * 0.1)),
            technical_terms_weight=max(0.01, current_weights.technical_terms_weight * (1 + feature_importance[1] * 0.2)),
            multi_step_weight=max(0.05, current_weights.multi_step_weight * (1 + feature_importance[2] * 0.15)),
            boolean_feature_weight=max(0.05, current_weights.boolean_feature_weight * (1 + feature_importance[4:].mean() * 0.1)),
            question_weight=max(0.05, current_weights.question_weight * (1 + feature_importance[3] * 0.1)),
        ...
        return OptimizationResult(
            ...
            training_metrics={
                ...
                'mse': mean_squared_error(y_test, np.mean(list(model_predictions.values()), axis=0)),
                'samples': len(features)
            },
            model_algorithm='ensemble_voting',
            cross_validation_score=ensemble_score,
            recommended_deployment=ensemble_score > self.calibration_config['confidence_threshold']
        )
    ...
    async def _simple_weight_optimization(self, training_data: Dict[str, Any]) -> OptimizationResult:
        ...
        new_weights = WeightConfiguration(
            word_count_weight=current.word_count_weight * (1 + correlations[0] * adjustment_factor),
            technical_terms_weight=current.technical_terms_weight * (1 + correlations[1] * adjustment_factor),
            multi_step_weight=current.multi_step_weight * (1 + correlations[2] * adjustment_factor),
            question_weight=current.question_weight * (1 + correlations[3] * adjustment_factor),
            boolean_feature_weight=current.boolean_feature_weight * (1 + np.mean(correlations[4:]) * adjustment_factor),
    ...
    async def deploy_optimized_weights(self, optimization_result: OptimizationResult) -> bool:
        """Deploy optimized weights with rollback capability"""
        if not optimization_result.recommended_deployment:
            self.logger.info(f"Optimization not recommended for deployment "
        ...
        try:
            ...
            self.logger.info(f"Deployed optimized weights v{optimization_result.new_weights.version} "
    ...
    async def _store_weight_configuration(self, optimization_result: OptimizationResult):
        ...
        try:
            ...
            cursor.execute("UPDATE think_calibration.weight_evolution SET is_active = FALSE")
    ...
    def _schedule_performance_monitoring(self, backup_weights: WeightConfiguration, deployment_time: datetime):
        ...
        def monitor_performance():
            ...
            if recent_accuracy < self.calibration_config['rollback_threshold']:
                self.logger.warning(f"Performance degraded to {recent_accuracy:.3f}, "
        ...
        def calibration_loop():
            while not self.shutdown_event.is_set():
                try:
                    ...
                    if (len(self.feedback_buffer) >= self.calibration_config['min_feedback_samples'] and
                        self.metrics['feedback_received'] % 100 == 0):  # Every 100 feedback samples
                        ...
                        if optimization and self.calibration_config['auto_deployment']:
                            loop.run_until_complete(self.deploy_optimized_weights(optimization))

                        loop.close()

                    # Sleep before next check
                    self.shutdown_event.wait(self.calibration_config['calibration_frequency'])
...
class CalibratedComplexityAnalyzer(NpuComplexityAnalyzer):
    ...
    def _calculate_complexity_score(self, features: ComplexityFeatures) -> float:
        ...
        score += min(features.technical_terms * weights.technical_terms_weight, weights.technical_terms_cap)

        # Calibrated multi-step complexity
        score += min(features.multi_step_indicators * weights.multi_step_weight, weights.multi_step_cap)
...
async def main():
    ...
    try:
        ...
        for i, task in enumerate(test_cases, 1):
            ...
            actual_complexity = complexity_score + np.random.normal(0, 0.1)  # Simulated feedback
            actual_decision = 'interleaved' if actual_complexity > 0.5 else 'no_thinking'

            # Record feedback
            from dynamic_think_mode_selector import ThinkModeAnalysis, ThinkModeDecision
            analysis = ThinkModeAnalysis(
                decision=ThinkModeDecision.INTERLEAVED if complexity_score > 0.5 else ThinkModeDecision.NO_THINKING,
        ...
        if metrics['performance']['feedback_received'] >= 3:
            ...
            if optimization:
                ...
                print(f"   Recommended: {'Yes' if optimization.recommended_deployment else 'No'}")
