...
def find_claude_code_installation() -> Optional[Path]:
    ...
    search_paths = [
        Path.home() / '.local' / 'npm-global' / 'lib' / 'node_modules' / '@anthropic-ai' / 'claude-code',
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if claude_path.exists():
                ...
                possible_root = claude_path.parent.parent / 'lib' / 'node_modules' / '@anthropic-ai' / 'claude-code'
...
def install_integration_module(claude_code_path: Path) -> bool:
    ...
    try:
        ...
        print(f"✓ Installed integration module: {target_file}")
        return True
        
    except Exception as e:
        print(f"✗ Failed to install integration module: {e}")
        return False
...
def create_config_files() -> bool:
    ...
    try:
        ...
        from claude_code_integration import create_claude_config, PROJECT_AGENTS
...
echo "✓ Project agents activated ({len(PROJECT_AGENTS)} agents)"
...
"""
        ...
        print(f"✓ Created activation script: {setup_script}")
...
"""
            ...
            if not post_commit_hook.exists():
                ...
                print("✓ Added git post-commit hook for auto-sync")
            
        return True
        
    except Exception as e:
        print(f"✗ Failed to setup auto-sync: {e}")
        return False
...
def verify_integration() -> bool:
    ...
    try:
        ...
        sys.path.insert(0, str(PROJECT_ROOT / 'src' / 'python'))
        from claude_code_integration import get_available_agents, get_agent_info
        
        agents = get_available_agents()
        print(f"✓ Verified {len(agents)} agents available")
        ...
        for agent in test_agents:
            if agent in agents:
                ...
                if info:
                    print(f"  ✓ {agent.upper()}: {info['description'][:50]}...")
...
def main():
    ...
    print("🚀 Installing Claude Code Project Agent Integration")
    print("=" * 60)
    ...
    if not claude_code_path:
        print("✗ Claude Code installation not found")
        print("  Please install Claude Code first: npm install -g @anthropic-ai/claude-code")
        return False
    
    if not quiet:
        print(f"✓ Found Claude Code installation: {claude_code_path}")
