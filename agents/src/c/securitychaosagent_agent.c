/*
 * SECURITYCHAOSAGENT AGENT - Communication System Integration
...
typedef struct {
    ...
    uint32_t agent_id;
    agent_state_t state;
} securitychaosagent_agent_t;

// Initialize agent
int securitychaosagent_init(securitychaosagent_agent_t* agent) {
    // Initialize communication context
    agent->comm_context = ufp_create_context("securitychaosagent");
    if (!agent->comm_context) {
        return -1;
    }
    
    strcpy(agent->name, "securitychaosagent");
    agent->state = AGENT_STATE_ACTIVE;
    
    // Register with discovery service
    agent_register("securitychaosagent", AGENT_TYPE_SECURITYCHAOSAGENT, NULL, 0);
...
// Main agent loop
void securitychaosagent_run(securitychaosagent_agent_t* agent) {
    ufp_message_t msg;
    
    while (agent->state == AGENT_STATE_ACTIVE) {
        // Receive messages
        if (ufp_receive(agent->comm_context, &msg, 100) == UFP_SUCCESS) {
            securitychaosagent_process_message(agent, &msg);
        }
    }
}
