/*
 * PYGUI AGENT - Communication System Integration
...
// Initialize agent
int pygui_init(pygui_agent_t* agent) {
    // Initialize communication context
    agent->comm_context = ufp_create_context("pygui");
    if (!agent->comm_context) {
        return -1;
    }
    
    strcpy(agent->name, "pygui");
    agent->state = AGENT_STATE_ACTIVE;
    
    // Register with discovery service
    agent_register("pygui", AGENT_TYPE_PYGUI, NULL, 0);
