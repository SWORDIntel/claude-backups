/*
 * PLANNER AGENT - Communication System Integration
...
// Initialize agent
int planner_init(planner_agent_t* agent) {
    // Initialize communication context
    agent->comm_context = ufp_create_context("planner");
    if (!agent->comm_context) {
        return -1;
    }
    
    strcpy(agent->name, "planner");
    agent->state = AGENT_STATE_ACTIVE;
    
    // Register with discovery service
    agent_register("planner", AGENT_TYPE_PLANNER, NULL, 0);
    
    return 0;
}

// Process incoming message
int planner_process_message(planner_agent_t* agent, ufp_message_t* msg) {
    // TODO: Implement agent-specific logic
    printf("PLANNER received message from %s\n", msg->source);
