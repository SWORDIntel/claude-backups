/*
 * TUI AGENT - Communication System Integration
...
// Initialize agent
int tui_init(tui_agent_t* agent) {
    // Initialize communication context
    agent->comm_context = ufp_create_context("tui");
    if (!agent->comm_context) {
        return -1;
    }
    
    strcpy(agent->name, "tui");
    agent->state = AGENT_STATE_ACTIVE;
    
    // Register with discovery service
    agent_register("tui", AGENT_TYPE_TUI, NULL, 0);
    
    return 0;
}

// Process incoming message
int tui_process_message(tui_agent_t* agent, ufp_message_t* msg) {
    // TODO: Implement agent-specific logic
    printf("TUI received message from %s\n", msg->source);
