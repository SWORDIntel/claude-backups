/*
 * MOBILE AGENT - Communication System Integration
...
// Initialize agent
int mobile_init(mobile_agent_t* agent) {
    // Initialize communication context
    agent->comm_context = ufp_create_context("mobile");
    if (!agent->comm_context) {
        return -1;
    }
    
    strcpy(agent->name, "mobile");
    agent->state = AGENT_STATE_ACTIVE;
    
    // Register with discovery service
    agent_register("mobile", AGENT_TYPE_MOBILE, NULL, 0);
