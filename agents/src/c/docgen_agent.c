/*
 * DOCGEN AGENT - Communication System Integration
...
// Initialize agent
int docgen_init(docgen_agent_t* agent) {
    // Initialize communication context
    agent->comm_context = ufp_create_context("docgen");
    if (!agent->comm_context) {
        return -1;
    }
    
    strcpy(agent->name, "docgen");
    agent->state = AGENT_STATE_ACTIVE;
    
    // Register with discovery service
    agent_register("docgen", AGENT_TYPE_DOCGEN, NULL, 0);
    
    return 0;
}

// Process incoming message
int docgen_process_message(docgen_agent_t* agent, ufp_message_t* msg) {
    // TODO: Implement agent-specific logic
    printf("Docgen received message from %s\n", msg->source);
