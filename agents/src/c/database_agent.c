/*
 * DATABASE AGENT - Communication System Integration
...
// Initialize agent
int database_init(database_agent_t* agent) {
    // Initialize communication context
    agent->comm_context = ufp_create_context("database");
    ...
    agent_register("database", AGENT_TYPE_DATABASE, NULL, 0);
    
    return 0;
}

// Process incoming message
int database_process_message(database_agent_t* agent, ufp_message_t* msg) {
    // TODO: Implement agent-specific logic
    printf("Database received message from %s\n", msg->source);
...
void database_run(database_agent_t* agent) {
    ...
    while (agent->state == AGENT_STATE_ACTIVE) {
        ...
        if (ufp_receive(agent->comm_context, &msg, 100) == UFP_SUCCESS) {
            database_process_message(agent, &msg);
        }
    }
}
