/*
 * MONITOR AGENT - Communication System Integration
...
int monitor_init(monitor_agent_t* agent) {
    ...
    agent->comm_context = ufp_create_context("monitor");
    ...
    agent_register("monitor", AGENT_TYPE_MONITOR, NULL, 0);
    
    return 0;
}

// Process incoming message
int monitor_process_message(monitor_agent_t* agent, ufp_message_t* msg) {
    // TODO: Implement agent-specific logic
    printf("Monitor received message from %s\n", msg->source);
...
void monitor_run(monitor_agent_t* agent) {
    ...
    while (agent->state == AGENT_STATE_ACTIVE) {
        ...
        if (ufp_receive(agent->comm_context, &msg, 100) == UFP_SUCCESS) {
            monitor_process_message(agent, &msg);
        }
    }
}
