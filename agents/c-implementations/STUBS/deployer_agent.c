/*
 * DEPLOYER AGENT - Communication System Integration
...
int deployer_init(deployer_agent_t* agent) {
    ...
    agent->comm_context = ufp_create_context("deployer");
    ...
    agent_register("deployer", AGENT_TYPE_DEPLOYER, NULL, 0);
    
    return 0;
}

// Process incoming message
int deployer_process_message(deployer_agent_t* agent, ufp_message_t* msg) {
    // TODO: Implement agent-specific logic
    printf("Deployer received message from %s\n", msg->source);
...
void deployer_run(deployer_agent_t* agent) {
    ...
    while (agent->state == AGENT_STATE_ACTIVE) {
        ...
        if (ufp_receive(agent->comm_context, &msg, 100) == UFP_SUCCESS) {
            deployer_process_message(agent, &msg);
        }
    }
}
