...
class DatabaseManager:
    ...
    def detect_postgres_port(self) -> int:
        ...
        print(
            f"{Colors.YELLOW}⚠ No active PostgreSQL detected, defaulting to port 5433{Colors.NC}"
        )
        return 5433
    ...
    def _test_port_connection(self, port: int) -> bool:
        ...
        try:
            result = subprocess.run(
                ["nc", "-z", "localhost", str(port)], capture_output=True, timeout=2
            )
                ...
                try:
                    ...
                    if result.returncode == 0:
                        ...
                        if self._test_port_connection(5433):
                            print(
                                f"{Colors.GREEN}✓ Local PostgreSQL started successfully{Colors.NC}"
                            )
                            return True
                    print(
                        f"{Colors.RED}✗ Failed to start local PostgreSQL: {result.stderr}{Colors.NC}"
                    )
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if version_match:
                ...
                if pg_version >= 13:
                    print(
                        f"{Colors.GREEN}✓ PostgreSQL {pg_version} connection successful{Colors.NC}"
                    )
                    return True
                else:
                    print(
                        f"{Colors.YELLOW}⚠ PostgreSQL {pg_version} detected - version 13+ recommended{Colors.NC}"
                    )
                    return True
            return True
    ...
    def create_database_if_not_exists(self) -> bool:
        ...
        try:
            ...
            if not exists:
                print(
                    f"{Colors.BLUE}Creating database '{self.config.db_name}'...{Colors.NC}"
                )
                cursor.execute(f'CREATE DATABASE "{self.config.db_name}"')
                print(f"{Colors.GREEN}✓ Database created successfully{Colors.NC}")
            else:
                print(
                    f"{Colors.GREEN}✓ Database '{self.config.db_name}' already exists{Colors.NC}"
                )
    ...
    def reset_database(self) -> bool:
        ...
        try:
            ...
            print(
                f"{Colors.YELLOW}Resetting database '{self.config.db_name}'...{Colors.NC}"
            )
    ...
    def setup_virtual_environment(self) -> bool:
        ...
        try:
            if not self.venv_path.exists():
                ...
                if result.returncode != 0:
                    print(
                        f"{Colors.RED}✗ Virtual environment creation failed: {result.stderr}{Colors.NC}"
                    )
                    return False
        ...
        for dep in dependencies:
            try:
                ...
                if result.returncode == 0:
                    ...
                    print(
                        f"{Colors.YELLOW}⚠ Failed to install {dep}: {result.stderr.strip()}{Colors.NC}"
                    )
    ...
    def _install_pytorch(self, python_executable: str):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print(
                    f"  {Colors.YELLOW}○ PyTorch installation skipped (optional){Colors.NC}"
                )

        except subprocess.TimeoutExpired:
            print(
                f"  {Colors.YELLOW}○ PyTorch installation timeout (optional){Colors.NC}"
            )
    ...
    def setup_learning_schema(self) -> bool:
        ...
        try:
            ...
            try:
                ...
                if result.returncode == 0:
                    print(
                        f"{Colors.GREEN}✓ Learning system schema setup completed{Colors.NC}"
                    )
                    if self.config.verbose:
                    ...
                    print(
                        f"{Colors.RED}✗ Schema setup failed: {result.stderr}{Colors.NC}"
                    )
                    return False
  ...
  • Configuration file generation
    ...
    def run_setup(self) -> bool:
        ...
        if not self.db_manager.start_local_postgres():
            print(
                f"{Colors.YELLOW}⚠ Local PostgreSQL not started - ensure PostgreSQL is running{Colors.NC}"
            )

        if not self.db_manager.test_connection():
            print(
                f"{Colors.RED}✗ Cannot connect to database. Check configuration.{Colors.NC}"
            )
            return False
        ...
        if not self._run_integration_tests():
            print(
                f"{Colors.YELLOW}⚠ Some integration tests failed, but setup is complete{Colors.NC}"
            )
    ...
    def _check_system_requirements(self) -> bool:
        ...
        if python_version >= (3, 8):
            print(
                f"{Colors.GREEN}✓ Python {python_version.major}.{python_version.minor} is compatible{Colors.NC}"
            )
        else:
            print(
                f"{Colors.RED}✗ Python 3.8+ required (found {python_version.major}.{python_version.minor}){Colors.NC}"
            )
        ...
        for cmd in required_commands:
            if subprocess.run(["which", cmd], capture_output=True).returncode == 0:
                ...
                print(
                    f"{Colors.YELLOW}⚠ {cmd} not found - some features may not work{Colors.NC}"
                )
    ...
    def _print_success_message(self):
        ...
        print(
            f"  python3 {self.config.python_dir}/postgresql_learning_system.py dashboard"
        )
