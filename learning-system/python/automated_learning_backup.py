...
class LearningDataBackupSystem:
    ...
    def __init__(self):
        ...
        db_user = os.getenv("POSTGRES_USER", "claude_agent")
        db_pass = os.getenv("POSTGRES_PASSWORD", "claude_auth_pass")
        db_host = os.getenv("POSTGRES_HOST", "localhost")
        db_port = os.getenv("POSTGRES_PORT", "5433")
        db_name = os.getenv("POSTGRES_DB", "claude_agents_auth")
        ...
        try:
            ...
            with gzip.open(backup_file, "wt") as gz_file:
                process = subprocess.run(
                    cmd, stdout=gz_file, stderr=subprocess.PIPE, text=True
                )

            if process.returncode == 0:
                file_size = backup_file.stat().st_size
                logger.info(
                    f"Database backup created: {backup_file} ({file_size} bytes)"
                )
    ...
    async def export_learning_data_json(self, days_back: int = 30) -> Dict[str, Any]:
        ...
        try:
            async with self.db_pool.acquire() as conn:
                ...
                logger.info(
                    f"Learning data exported: {export_file} ({file_size} bytes)"
                )
    ...
    async def backup_log_files(self) -> Dict[str, Any]:
        ...
        if not log_dir.exists() or not list(log_dir.glob("*.jsonl")):
            return {"success": False, "error": "No log files found to backup"}
        ...
        try:
            ...
            logger.info(
                f"Log files backed up: {log_backup_file} ({file_size} bytes, {log_count} files)"
            )
    ...
    async def cleanup_old_backups(self, keep_days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Cleaned up {len(cleaned_files)} old backup files, freed {total_size_freed} bytes"
            )
    ...
    async def run_comprehensive_backup(self) -> Dict[str, Any]:
        ...
        results = {"backup_timestamp": datetime.utcnow().isoformat(), "operations": {}}
        ...
        # Calculate summary
        successful_ops = sum(
            1 for op in results["operations"].values() if op.get("success")
        )
        ...
        logger.info(
            f"Comprehensive backup completed: {successful_ops}/{total_ops} operations successful"
        )
...
async def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "full":
            ...
            print(
                "Available commands: full, database, json [days], csv [days], logs, cleanup [days]"
            )
            return
