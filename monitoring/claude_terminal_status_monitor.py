...
class ClaudeTerminalStatusMonitor:
    ...
    def position_via_xdotool(self):
        ...
        if result.returncode == 0:
            ...
            if width and height:
                ...
                self.root.geometry(f"{self.window_width}x{self.window_height}+{status_x}+{status_y}")
                return True
        return False

    def position_via_wmctrl(self):
        """Try positioning using wmctrl"""
        result = subprocess.run(['wmctrl', '-l', '-G'], capture_output=True, text=True, timeout=2)
        if result.returncode == 0:
            lines = result.stdout.strip().split('\n')
            for line in lines:
                if any(term in line.lower() for term in ['claude', 'terminal', 'gnome-terminal']):
                    parts = line.split()
                    if len(parts) >= 6:
    ...
    def setup_ui(self):
        ...
        # Title
        title_label = ttk.Label(main_frame, text="Claude Agent Framework",
                               style='Title.TLabel')
        title_label.grid(row=0, column=0, columnspan=2, pady=(0, 8), sticky=tk.W)

        # Close button (small, top-right)
        close_btn = ttk.Button(main_frame, text="×", width=3,
                              style='Dark.TButton', command=self.close_monitor)
        ...
        # Framework status
        ttk.Label(main_frame, text="Framework:", style='Dark.TLabel').grid(row=row, column=0, sticky=tk.W, pady=2)
        self.status_labels['framework'] = ttk.Label(main_frame, text="●", style='Status.TLabel')
        self.status_labels['framework'].grid(row=row, column=1, sticky=tk.W, pady=2)
        row += 1

        # Communication system
        ttk.Label(main_frame, text="Communication:", style='Dark.TLabel').grid(row=row, column=0, sticky=tk.W, pady=2)
        self.status_labels['communication'] = ttk.Label(main_frame, text="●", style='Status.TLabel')
        self.status_labels['communication'].grid(row=row, column=1, sticky=tk.W, pady=2)
        row += 1

        # Database status
        ttk.Label(main_frame, text="Database:", style='Dark.TLabel').grid(row=row, column=0, sticky=tk.W, pady=2)
        self.status_labels['database'] = ttk.Label(main_frame, text="●", style='Status.TLabel')
        self.status_labels['database'].grid(row=row, column=1, sticky=tk.W, pady=2)
        row += 1

        # Agents status
        ttk.Label(main_frame, text="Agents:", style='Dark.TLabel').grid(row=row, column=0, sticky=tk.W, pady=2)
        self.status_labels['agents'] = ttk.Label(main_frame, text="0/98", style='Dark.TLabel')
        self.status_labels['agents'].grid(row=row, column=1, sticky=tk.W, pady=2)
        row += 1

        # Modules status
        ttk.Label(main_frame, text="Modules:", style='Dark.TLabel').grid(row=row, column=0, sticky=tk.W, pady=2)
        self.status_labels['modules'] = ttk.Label(main_frame, text="0/11", style='Dark.TLabel')
        self.status_labels['modules'].grid(row=row, column=1, sticky=tk.W, pady=2)
        row += 1

        # OpenVINO status
        ttk.Label(main_frame, text="OpenVINO:", style='Dark.TLabel').grid(row=row, column=0, sticky=tk.W, pady=2)
        self.status_labels['openvino'] = ttk.Label(main_frame, text="●", style='Status.TLabel')
        self.status_labels['openvino'].grid(row=row, column=1, sticky=tk.W, pady=2)
        row += 1

        # NPU status
        ttk.Label(main_frame, text="NPU:", style='Dark.TLabel').grid(row=row, column=0, sticky=tk.W, pady=2)
        self.status_labels['npu'] = ttk.Label(main_frame, text="Checking...", style='Dark.TLabel')
        self.status_labels['npu'].grid(row=row, column=1, sticky=tk.W, pady=2)
        row += 1

        # Shadowgit status
        ttk.Label(main_frame, text="Shadowgit:", style='Dark.TLabel').grid(row=row, column=0, sticky=tk.W, pady=2)
        self.status_labels['shadowgit'] = ttk.Label(main_frame, text="●", style='Status.TLabel')
        self.status_labels['shadowgit'].grid(row=row, column=1, sticky=tk.W, pady=2)
        row += 1

        # Hardware status
        ttk.Label(main_frame, text="CPU Temp:", style='Dark.TLabel').grid(row=row, column=0, sticky=tk.W, pady=2)
        self.status_labels['temp'] = ttk.Label(main_frame, text="--°C", style='Dark.TLabel')
        self.status_labels['temp'].grid(row=row, column=1, sticky=tk.W, pady=2)
        row += 1

        # Memory usage
        ttk.Label(main_frame, text="Memory:", style='Dark.TLabel').grid(row=row, column=0, sticky=tk.W, pady=2)
        self.status_labels['memory'] = ttk.Label(main_frame, text="--%", style='Dark.TLabel')
        self.status_labels['memory'].grid(row=row, column=1, sticky=tk.W, pady=2)
        row += 1

        # Last update
        self.status_labels['update'] = ttk.Label(main_frame, text="Initializing...",
                                                style='Dark.TLabel')
    ...
    def check_agent_status(self):
        ...
        project_registry = Path('/home/john/claude-backups/config/registered_agents.json')
    ...
    def check_communication_system(self):
        ...
        try:
            ...
            if c_engine.exists():
                ...
                compiled_files = list(c_engine.glob('*.so')) + list(c_engine.glob('*.a'))
    ...
    def update_status_display(self):
        ...
        if self.status_data['agents_online'] > 80:  # Most agents online
            self.status_labels['framework'].config(text="●", foreground=self.colors['success'])
        elif self.status_data['agents_online'] > 50:
            self.status_labels['framework'].config(text="●", foreground=self.colors['warning'])
        else:
            self.status_labels['framework'].config(text="●", foreground=self.colors['error'])
        ...
        self.status_labels['modules'].config(text=modules_text, foreground=modules_color)

        # OpenVINO status
        openvino_status = self.status_data.get('modules_detail', {}).get('OpenVINO', False)
        openvino_color = self.colors['success'] if openvino_status else self.colors['error']
        ...
        # Shadowgit status
        shadowgit_status = self.status_data.get('modules_detail', {}).get('Shadowgit', False)
        shadowgit_color = self.colors['success'] if shadowgit_status else self.colors['error']
        self.status_labels['shadowgit'].config(text="●", foreground=shadowgit_color)

        # Hardware status with dark theme colors
        temp_text = f"{self.status_data['cpu_temp']:.0f}°C" if self.status_data['cpu_temp'] > 0 else "--°C"
        ...
        mem_text = f"{self.status_data['memory_usage']:.0f}%" if self.status_data['memory_usage'] > 0 else "--%"
        ...
        if self.status_data['last_update']:
            update_text = f"Updated: {self.status_data['last_update'].strftime('%H:%M:%S')}"
            self.status_labels['update'].config(text=update_text, foreground=self.colors['secondary'])
    ...
    def monitor_loop(self):
        ...
        while True:
            try:
                ...
                database_status = self.status_data.get('modules_detail', {}).get('PostgreSQL', False)

                # Debug output
                print(f"[DEBUG] Agents: {agents_online}, Modules: {modules_online}, CPU: {cpu_temp:.1f}°C, Mem: {memory_usage:.1f}%")
                ...
                self.status_data.update({
                    ...
                    'communication_status': 'Online' if communication_status else 'Offline',
