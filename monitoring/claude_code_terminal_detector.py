...
class ClaudeCodeTerminalDetector:
    ...
    def calculate_status_position(self, terminal_geometry, status_width=280, status_height=200):
        """Calculate optimal position for status window in terminal's top-right"""
        if not terminal_geometry:
            return None

        # Position in top-right corner with some padding
        status_x = terminal_geometry['x'] + terminal_geometry['width'] - status_width - 20
...
if __name__ == "__main__":
    ...
    if position:
        print(f"Optimal position: {position['width']}x{position['height']}+{position['x']}+{position['y']}")
    else:
