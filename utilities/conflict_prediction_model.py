...
class ConflictPredictionModel:
    ...
    async def extract_features(self, repo_data: Dict[str, Any]) -> np.ndarray:
        ...
        features.extend(
            [
                ...
                (datetime.now() - commit_time).total_seconds()
                / 3600,  # Hours since commit
            ]
        )
        ...
        features.extend(
            [
                ...
                min(lines_added, lines_deleted)
                / max(max(lines_added, lines_deleted), 1),  # Change balance
            ]
        )
    ...
    async def train_models(self) -> Dict[str, float]:
        ...
        if ensemble_predictions:
            ...
            logger.info(
                f"Ensemble - Accuracy: {ensemble_accuracy:.4f}, F1: {ensemble_f1:.4f}"
            )
        ...
        try:
            ...
            for name, model in self.models.items():
                try:
                    ...
                    conflict_prob = (
                        pred_proba[1] if len(pred_proba) > 1 else pred_proba[0]
                    )
