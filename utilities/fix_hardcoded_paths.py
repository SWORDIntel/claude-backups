...
class PathFixer:
    ...
    def fix_imports_section(self, content: str) -> str:
        ...
        has_path_utilities = any(
            "from path_utilities import" in line or "import path_utilities" in line
            for line in lines
        )
        has_project_root_setup = any(
            "project_root = Path(__file__)" in line for line in lines
        )
        ...
        if insert_index == -1:
            for i in range(len(lines) - 1, -1, -1):
                if ("import " in lines[i] or "from " in lines[i]) and not lines[
                    i
                ].strip().startswith("#"):
                    insert_index = i + 1
                    break
    ...
    def fix_hardcoded_paths(self, content: str) -> Tuple[str, List[str]]:
        ...
        patterns_john = [
            ...
            (r"Path\('/home/john/claude-backups'\)", "get_project_root()"),
            (
                r'Path\("/home/john/claude-backups/([^"]+)"\)',
                r'get_project_root() / "\1"',
            ),
            (
                r"Path\('/home/john/claude-backups/([^']+)'\)",
                r"get_project_root() / '\1'",
        ...
        patterns_ubuntu = [
            ...
            (r"Path\('/home/ubuntu/claude-backups'\)", "get_project_root()"),
            (
                r'Path\("/home/ubuntu/claude-backups/([^"]+)"\)',
                r'get_project_root() / "\1"',
            ),
            (
                r"Path\('/home/ubuntu/claude-backups/([^']+)'\)",
        ...
        # Apply all patterns
        all_patterns = (
            patterns_john + patterns_ubuntu + patterns_docs + patterns_shadowgit
        )
        ...
        try:
            ...
            if content != original_content:
                ...
                self.changes_made.extend(
                    [f"{file_path}: {change}" for change in changes]
                )
