...
def test_trie_performance():
    ...
    for query in queries:
        ...
        print(
            f"  Lines: {result['total_lines']} | Relevant: {result['relevant_lines']}"
        )
        print(f"  Patterns: {', '.join(result['patterns_found'][:5])}")
        print(
            f"  Trie: {result['trie_time_ms']:.2f}ms | Total: {result['total_time_ms']:.2f}ms"
        )
        print()
...
def test_integrated_performance():
    ...
    for i in range(num_files):
        ...
        result = optimizer.process_with_trie_optimization(
            test_content, "find errors and functions"
        )
    ...
    top_indices, top_scores = avx2_ops.find_top_k_similar(
        query_embedding, chunk_embeddings, k=100
    )
    ...
    print(
        f"  Trie matching: {trie_time*1000:.2f}ms ({num_files/trie_time:.0f} files/sec)"
    )
    print(
        f"  Vector search: {vector_time*1000:.2f}ms ({len(chunk_embeddings)/vector_time:.0f} vectors/sec)"
    )
...
def compare_before_after():
    ...
    vector_improvement = (
        old_vector_time / new_vector_time if new_vector_time > 0 else float("inf")
    )
