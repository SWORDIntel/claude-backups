        ...
        for pattern in capability_patterns:
            if pattern in content:
                ...
                if capability_section:
                    capabilities.extend(
                        [item.strip() for item in capability_section if item.strip()]
                    )
                    break
...
def infer_category_from_name(agent_name: str) -> str:
    ...
    if any(word in name_lower for word in ["director", "orchestrator", "planner"]):
        ...
        word in name_lower
        for word in ["infrastructure", "deployer", "monitor", "docker", "proxmox"]
    ):
        return "INFRASTRUCTURE"
    elif any(
        word in name_lower for word in ["web", "mobile", "android", "gui", "tui", "api"]
    ):
        return "PLATFORMS"
    elif any(
        word in name_lower
...
def scan_agents_directory(agents_dir: Path) -> List[Dict[str, Any]]:
    ...
    agent_files = [
        f for f in agents_dir.iterdir() if f.is_file() and f.suffix.lower() == ".md"
    ]
    ...
    def is_agent_file(filename: str) -> bool:
        ...
        if (
            ...
            or "agent" in name
            or any(
                keyword in name for keyword in ["internal", "orchestrator", "director"]
            )
        ):
            return True
...
def generate_statistics(agents: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "avg_health_score": (
            sum(a["health"] for a in agents) / total_agents if total_agents > 0 else 0
        ),
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--agents-dir", default="./agents", help="Path to agents directory"
    )
    parser.add_argument(
        "--output", default="./config/agent-registry.json", help="Output registry file"
    )
    parser.add_argument("--print", action="store_true", help="Print registry to stdout")

    args = parser.parse_args()

    try:
        registry = create_agent_registry(
            args.agents_dir, args.output if not args.print else None
        )
