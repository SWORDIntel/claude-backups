    ...
    async with initialization_lock:
        try:
            if PostgreSQLLearningSystem:
                ...
                await asyncio.wait_for(learning_system.initialize_async(), timeout=30.0)

                system_status["initialized"] = True
                system_status["ml_available"] = (
                    hasattr(learning_system, "model")
                    and learning_system.model is not None
                )
                system_status["database_connected"] = (
                    learning_system.db_connection is not None
                )
                system_status["last_heartbeat"] = datetime.now().isoformat()
                system_status["error_message"] = None

                logger.info(
                    f"Learning system initialized: ML={system_status['ml_available']}, DB={system_status['database_connected']}"
                )
            else:
...
async def record_performance(
    request: AgentPerformanceRequest, background_tasks: BackgroundTasks
):
...
@app.get(
    "/agent/{agent_id}/recommendations", response_model=AgentRecommendationResponse
)
