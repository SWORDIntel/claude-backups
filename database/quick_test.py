...
async def test_database():
    ...
    try:
        ...
        auth_result = await conn.fetchrow("SELECT * FROM authenticate_user($1, $2)", 'admin', 'dummy_hash')
        auth_time = (time.time() - start_time) * 1000
        print(f"✅ Authentication function test: {auth_time:.2f}ms")
        
        # Test performance metrics view
        try:
            perf_metrics = await conn.fetch("SELECT * FROM auth_performance_metrics LIMIT 3;")
