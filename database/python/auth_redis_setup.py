...
class AuthRedisManager:
    ...
    async def store_session(self, session_id: str, session_data: SessionData) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"Stored session {session_id} for user {session_data.username}"
            )
            return True
    ...
    async def cache_user_permissions(
        ...
        try:
            ...
            await self.redis_client.setex(
                key, self.PERMISSION_CACHE_TTL, json.dumps(data)
            )
    ...
    async def cache_role_permissions(
        ...
        try:
            ...
            await self.redis_client.setex(
                key, self.ROLE_CACHE_TTL, json.dumps(permissions)
            )
            return True
    ...
    async def check_rate_limit(
        ...
        try:
            ...
            # Check if blocked
            block_key = (
                f"{self.RATE_BUCKET_PREFIX}:{identifier_type}:{identifier}:blocked"
            )
    ...
    async def get_cache_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for prefix in [
                self.SESSION_PREFIX,
                self.TOKEN_BLACKLIST_PREFIX,
                self.USER_PERMS_PREFIX,
            ...
            return {
                ...
                "hit_ratio": float(info.get("keyspace_hits", 0))
                / max(
...
async def setup_redis_auth_cache():
    ...
    try:
        ...
        await redis_manager.blacklist_token(
            test_jti, datetime.now() + timedelta(hours=1)
        )
