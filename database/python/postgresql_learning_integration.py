...
try:
    ...
    print("Warning: Install PostgreSQL drivers with: pip install asyncpg psycopg2-binary")
...
class DatabaseLearningIntegration:
    ...
    async def check_existing_schema(self):
        ...
        try:
            ...
            return {
                ...
                'missing_tables': [t for t in ['users', 'user_sessions', 'roles', 'permissions'] if t not in auth_tables],
    ...
    async def setup_learning_integration(self):
        ...
        try:
            ...
            schema_path = Path(__file__).parent.parent / 'sql' / 'learning_system_schema.sql'
    ...
    async def verify_integration(self):
        ...
        try:
            ...
            return {
                ...
                'learning_functions': [row['routine_name'] for row in learning_functions],
                'materialized_views': [row['matviewname'] for row in materialized_views],
                'test_results': test_results,
                'integration_status': 'success' if all('FAILED' not in str(v) for v in test_results.values()) else 'partial'
            }
    ...
    async def get_database_stats(self):
        ...
        try:
            ...
            return {
                ...
                'learning_system': dict(learning_stats) if isinstance(learning_stats, dict) else learning_stats
            }
...
async def main():
    ...
    if 'error' not in stats['learning_system']:
        print(f"   Learning executions: {stats['learning_system'].get('total_executions', 0)}")
        print(f"   Tracked agents: {stats['learning_system'].get('tracked_agents', 0)}")
    print()
    
    # Create sample data (optional)
    create_sample = input("Create sample learning data for testing? (y/N): ").lower().strip()
    if create_sample == 'y':
