...
class UnifiedAgentRegistry:
    ...
    def __init__(
        self, project_root: Optional[Path] = None, agents_dir: Optional[Path] = None
    ):
    ...
    def _load_from_claude_code_integration(self):
        ...
        try:
            integration_file = (
                self.agents_dir / "src" / "python" / "claude_code_integration.py"
            )
            ...
            spec = importlib.util.spec_from_file_location(
                "claude_code_integration", integration_file
            )
    ...
    def find_agents_by_pattern(self, pattern: str) -> List[str]:
        ...
        for name, agent in self.agents.items():
            if (
                ...
                or pattern in agent.description.lower()
                or any(
                    pattern in trigger.lower() for trigger in agent.proactive_triggers
                )
            ):
                matches.append(name)
...
"""
    ...
    def _handle_post_task(self) -> List[Dict[str, Any]]:
        ...
        actions.append(
            {"type": "learning_update", "status": "success", "task_completed": True}
        )
...
class UnifiedAgentInvoker:
    ...
    def _load_orchestrator(self):
        ...
        try:
            ...
            if orchestrator_file.exists():
                spec = importlib.util.spec_from_file_location(
                    "production_orchestrator", orchestrator_file
                )
                module = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(module)
    ...
    def invoke_agent(
        ...
        agent_name: str,
        prompt: str,
        method: Optional[AgentInvocationMethod] = None,
    ) -> Dict[str, Any]:
    ...
    def _invoke_via_command(
        self, agent: UnifiedAgentMetadata, prompt: str
    ) -> Dict[str, Any]:
    ...
    def _invoke_via_python(
        ...
        try:
            spec = importlib.util.spec_from_file_location(
                agent.name, agent.python_impl_path
            )
            module = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(module)
            ...
            if not AgentClass:
                ...
                if hasattr(module, class_name_upper):
                    ...
                    raise ImportError(
                        f"Could not find {class_name} or {class_name_upper} in {agent.python_impl_path}"
                    )
            ...
            if not hasattr(instance, "execute_command"):
                raise NotImplementedError(
                    f"'execute_command' not found in {AgentClass.__name__}"
                )
    ...
    def _invoke_via_orchestrator(
        ...
        try:
            ...
            if hasattr(self.orchestrator, "schedule_task"):
                ...
                raise NotImplementedError(
                    "Orchestrator does not have a 'schedule_task' method"
                )
...
class UnifiedClaudeCodeIntegration:
    ...
    def __init__(
        ...
        self.registry = UnifiedAgentRegistry(
            project_root=project_root, agents_dir=agents_dir
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Unified Claude Code Integration System"
    )
    parser.add_argument("--trigger", help="Trigger a hook phase")
    parser.add_argument(
        "--invoke", help="Invoke an agent", nargs=2, metavar=("AGENT", "PROMPT")
    )
