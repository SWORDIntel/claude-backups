...
def test_hybrid_bridge_manager():
    ...
    try:
        ...
        print_info(
            f"Bridge Status: {status.get('bridge_manager', {}).get('status', 'unknown')}"
        )
        print_info(
            f"Current Mode: {status.get('bridge_manager', {}).get('mode', 'unknown')}"
        )
        print_info(
            f"Native System: {status.get('systems', {}).get('native', {}).get('available', False)}"
        )
        print_info(
            f"Docker System: {status.get('systems', {}).get('docker', {}).get('available', False)}"
        )
...
def test_learning_system():
    ...
    try:
        ...
        try:
            ...
            print_warning(
                f"Learning system init failed (DB connection issue): {init_error}"
            )
...
def test_integration():
    ...
    try:
        ...
        try:
            ...
            print_warning(
                "Learning system DB connection issue (expected in test environment)"
            )
...
def generate_report(results):
    ...
    print(
        f"  Success Rate: {GREEN if success_rate >= 80 else YELLOW if success_rate >= 60 else RED}{success_rate:.1f}%{NC}"
    )
    ...
    if success_rate >= 80:
        ...
        print_warning(
            "PARTIAL SUCCESS - Core functionality operational with some limitations"
        )
    ...
    if not results.get("Environment", False):
        print_warning(
            "Install missing Python dependencies: pip install psycopg2-binary asyncpg numpy"
        )

    if not results.get("Docker Configuration", False):
        print_info(
            "Consider installing Docker for enhanced containerization capabilities"
        )
...
def main():
    ...
    success_rate = (
        sum(1 for result in results.values() if result) / len(results)
    ) * 100
