...
def test_tandem_integration():
    ...
    try:
        from tandem_integration import enhance_agent, EnhancedAgentWrapper
        ...
        class SimpleAgent:
            ...
            def execute_command(self, command):
                return {"status": "success", "result": f"Executed: {command.get('action', 'unknown')}"}
        ...
        if hasattr(enhanced_agent, 'original_methods'):
            print(f"  ✅ Original methods extracted: {list(enhanced_agent.original_methods.keys())}")
        else:
            ...
            for attr_name in dir(module):
                if 'executor' in attr_name.lower() and not attr_name.startswith('_'):
                    ...
                    if hasattr(executor_class, '__call__'):
                        ...
                        enhanced_agent = enhance_agent(original_agent, agent_name.upper(), "mock")
                        print(f"  ✅ Enhanced {agent_name}: {enhanced_agent.agent_name}")
                        enhanced_count += 1
                        break
