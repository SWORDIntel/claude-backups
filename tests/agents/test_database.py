...
async def test_database_via_orchestrator():
    ...
    try:
        from production_orchestrator import CommandStep, ProductionOrchestrator
        ...
        database_result = await orchestrator._invoke_real_agent(
            database_step, orchestrator.get_agent_info("database")
        )
...
async def test_database_postgresql17():
    ...
    try:
        ...
        print(f"   Status: {result.get('status')}")
        print(
            f"   Features: {len(result.get('result', {}).get('postgresql17_features', {}))}"
        )
