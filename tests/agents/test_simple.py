...
try:
    ...
    from ENHANCED_AGENT_INTEGRATION import AgentStatus, Priority
    ...
    test_command = CommandSet(
        ...
        steps=[CommandStep(action="test", agent="Director", payload={"test": True})],
    )
    ...
    print(
        f"✅ Generated DAG with {len(dag['nodes'])} nodes and {len(dag['edges'])} edges"
    )
