...
async def test_security_agent_direct():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        print(
            f"Vulnerabilities Found: {result['result'].get('vulnerabilities_found', 0)}"
        )
        ...
        result = await security.execute_command(
            "compliance_audit", {"framework": "owasp", "target": temp_dir}
        )
        ...
        result = await security.execute_command("create_policy", {"type": "password"})
...
async def main():
    ...
    if security_status:
        print(
            f"  - Security Audits Performed: {security_status['metrics']['security_audits']}"
        )
        print(
            f"  - Vulnerabilities Found: {security_status['metrics']['vulnerabilities_found']}"
        )
        print(
            f"  - Compliance Checks: {security_status['metrics']['compliance_checks']}"
        )
        print(
            f"  - Threat Models Created: {security_status['metrics']['threat_models_created']}"
        )
        print(
            f"  - Overall Security Score: {security_status['metrics']['security_score']}"
        )
    ...
    if security_status and security_status["metrics"]["security_audits"] > 0:
        ...
        print(
            f"✓ Successfully executed {security_status['metrics']['security_audits']} security audits"
        )
        print(
            f"✓ Found {security_status['metrics']['vulnerabilities_found']} vulnerabilities"
        )
        ...
        print(
            f"The Security agent is fully functional and actively performing security analysis."
        )
    else:
