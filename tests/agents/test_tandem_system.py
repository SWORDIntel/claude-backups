    ...
    for name, mode in test_workflows:
        try:
            workflow = CommandSet(
                ...
                dependencies={"design": ["plan"], "implement": ["design"]},
            )
...
async def test_standard_workflows():
    ...
    workflows = [
        (
            "Document Generation",
            StandardWorkflows.create_document_generation_workflow(),
        ),
    ...
    for name, workflow in workflows:
        try:
            ...
            print(
                f"✅ {name}: {status} ({steps_completed} steps) in {execution_time:.2f}s"
            )
            results.append(status == "completed")
...
async def test_agent_discovery():
    ...
    for query, expected_types in test_queries:
        ...
        found_expected = any(
            any(exp in agent.lower() for exp in expected_types) for agent in agents
        )
        results.append(
            found_expected or len(agents) > 0
        )  # Success if expected found OR any agents found
...
async def quick_demo():
    ...
    result = await orchestrator.invoke_agent(
        "director", "assess_project", {"complexity": "medium"}
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--comprehensive", "-c", action="store_true", help="Run comprehensive tests"
    )
