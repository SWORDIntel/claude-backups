...
async def test_quantumguard_direct():
    ...
    try:
        ...
        result = await executor.execute_command(
            "implement_pqc", {"algorithm": "kyber768", "operation": "keygen"}
        )
...
async def test_security_direct():
    ...
    try:
        ...
        result = await executor.execute_command(
            "vulnerability_scan", {"target": ".", "type": "sast"}
        )
...
async def test_agents_via_orchestrator():
    ...
    try:
        ...
        quantumguard_step = CommandStep(
            agent="quantumguard", action="deploy_pqc", payload={"algorithm": "kyber768"}
        )

        quantumguard_result = await orchestrator._invoke_real_agent(
            quantumguard_step, orchestrator.get_agent_info("quantumguard")
        )
        ...
        security_result = await orchestrator._invoke_real_agent(
            security_step, orchestrator.get_agent_info("security")
        )
...
async def test_security_workflow():
    ...
    try:
        ...
        quantumguard_executed = any(
            "quantumguard" in str(step_result.get("agent", ""))
            for step_result in results.values()
        )
        security_executed = any(
            "security" in str(step_result.get("agent", ""))
            for step_result in results.values()
        )
