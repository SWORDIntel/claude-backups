...
def main():
    ...
    test_repos = [
        ...
        (
            Path("/home/ubuntu/Documents/ARTICBASTION/docs"),
    ...
    for result in results:
        ...
        if repo_total > 0:
            repo_roles = sorted(
                result["role_mappings"].items(), key=lambda x: len(x[1]), reverse=True
            )
    ...
    print(
        f"🎉 Successfully categorized {total_assignments} documents across 8 role categories"
    )
