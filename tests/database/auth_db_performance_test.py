...
class AuthDatabasePerformanceTest:
        ...
        async with self.db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                if not agent_role_id:
                    raise RuntimeError(
                        "Agent role not found. Run ../sql/auth_db_setup.sql first."
                    )
    ...
    async def test_authentication_performance(self) -> PerformanceMetrics:
        ...
        tasks = [
            auth_operation()
            for _ in range(
                self.config.concurrent_users * self.config.operations_per_user
            )
        ]
        ...
        return PerformanceMetrics(
            ...
            throughput_ops_per_sec=(
                len(tasks) / total_duration if total_duration > 0 else 0
            ),
    ...
    async def test_redis_caching_performance(self) -> PerformanceMetrics:
        ...
        for user in self.test_users[:100]:
            ...
            await redis_client.set(
                cache_key, json.dumps(permissions), ex=900
            )  # 15 min TTL
    ...
    def print_metrics(self, metrics: PerformanceMetrics):
        ...
        if "Authentication" in metrics.test_name:
            print(
                f"  P95 < 50ms:         {'✓ PASS' if metrics.p95_latency_ms < 50 else '✗ FAIL'} ({metrics.p95_latency_ms:.2f}ms)"
            )
            print(
                f"  Throughput >1000:   {'✓ PASS' if metrics.throughput_ops_per_sec > 1000 else '✗ FAIL'} ({metrics.throughput_ops_per_sec:.0f} ops/sec)"
            )

        elif "Session" in metrics.test_name or "Permission" in metrics.test_name:
            print(
                f"  P95 < 20ms:         {'✓ PASS' if metrics.p95_latency_ms < 20 else '✗ FAIL'} ({metrics.p95_latency_ms:.2f}ms)"
            )

        elif "Concurrent" in metrics.test_name:
            print(
                f"  Success Rate >95%:  {'✓ PASS' if metrics.success_rate > 95 else '✗ FAIL'} ({metrics.success_rate:.2f}%)"
            )

        elif "Redis" in metrics.test_name:
            print(
                f"  P95 < 5ms:          {'✓ PASS' if metrics.p95_latency_ms < 5 else '✗ FAIL'} ({metrics.p95_latency_ms:.2f}ms)"
            )
...
async def main():
    ...
    # Test configuration
    config = TestConfig(concurrent_users=1000, operations_per_user=10)
    ...
    try:
        ...
        for result in results:
            ...
            if "Authentication" in result.test_name:
                test_pass = (
                    result.p95_latency_ms < 50
                    and result.throughput_ops_per_sec > 1000
        ...
        print(
            f"Database is {'READY FOR PRODUCTION' if overall_pass else 'NOT READY - OPTIMIZATION REQUIRED'}"
        )
