    ...
    try:
        ...
        if LEARNING_SYSTEM_AVAILABLE:
            ...
            try:
                ...
                if success:
                    ...
                    print(
                        f"   📊 Detected PostgreSQL version: {health.get('postgres_version')}"
                    )
                    print(
                        f"   🔧 Database integration: {health.get('database_integration')}"
                    )
                    print(
                        f"   ✅ Version compatibility: {health.get('version_compatibility')}"
                    )

                    features = health.get("postgres_features", {})
                    print(
                        f"   🚀 JSON_ARRAY available: {features.get('json_array_function', False)}"
                    )
                    print(
                        f"   🚀 JSON_OBJECT available: {features.get('json_object_function', False)}"
                    )
            ...
            print(
                "   💡 Run from the agents/src/python directory to test the learning system"
            )
        ...
        if version_number >= 17.0:
            if json_array_available and json_object_available:
                ...
                print(
                    "   ⚠️  Status: PostgreSQL 17 detected but JSON functions not working"
                )
